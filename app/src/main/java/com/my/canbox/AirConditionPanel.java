...
//	public static final int MESSAGE_AIR_HIDE = 0x03;
	...
	private byte []mAirData = new byte[15];
...
//		}
	...
	private byte[] mAirDataBackup = new byte[15];
...
//			Log.d("ffcc", ""+mAirDataBackupEmpty);
			...
			if (GlobalDef.mRudder) {// change temp
				byte temp = mAirData[2];
				mAirData[2] = mAirData[3];
				mAirData[3] = temp;
				//return;
				...
				if (!"com.canboxsetting/com.canboxsetting.CanAirControlActivity"
					...
					if (!off && !dataEqual && !CarUtil.isHideAirCondition()) {
						UtilSystem.doRunActivity(mContext, "com.canboxsetting",
								"com.canboxsetting.CanAirControlActivity");

						removeMessages(MESSAGE_AIR_TO_ACCONTROL_APK);
						removeMessages(MESSAGE_AIR_TO_ACCONTROL_APK);
						sendMessageDelayed(
								obtainMessage(MESSAGE_AIR_TO_ACCONTROL_APK,
										msg.obj), 600);
...
//				mToast.show();
				if (!dataEqual && !off) {
					...
					if (airConditionView != null
							&& airConditionView.getParent() == null) {
...
//		mToast.setView(airConditionView);	
    ...
    void setAirCondtionTitle(View view){   	
		...
		if ((mAirData[5] & 0x10) != 0) {
			view.findViewById(R.id.air_fwindow_heat)
...
//		}
    ...
    void setAirCondtionTemperature(View view){
    	...
    	if(outDoorTemp!= 0xff){
    		...
    		((TextView)view.findViewById(R.id.outdoor_temp)).setText(view.getResources().getString(R.string.outdoor_temp)+
		...
		if ((seatHeat != mSeatHeat)
				|| (seatCold != mSeatCold)) {
			mSeatHeat = (mAirData[4] & 0x33);
			view.findViewById(R.id.air_action_seat).setVisibility(View.VISIBLE);
			if ((seatHeat & 0x44)!=0){
				((ImageView) view.findViewById(R.id.seat_heat_right_4)).setVisibility(View.VISIBLE);
				((ImageView) view.findViewById(R.id.seat_heat_left_4)).setVisibility(View.VISIBLE);
			} 
