	...
	public static void init(Context context) {
		if (mView == null) {
			mView = ((LayoutInflater) context
					...
					.inflate(R.layout.warning_msg, null);
...
//			mLayoutParams.gravity = Gravity.LEFT | Gravity.TOP;
			...
			mView.findViewById(R.id.warning_ok).setOnClickListener(
					new OnClickListener() {
			...
			mImageWarn2 = (ImageView) mView.findViewById(R.id.waring_image2);
	...
	public static void updateView(Context context, int drawable, int drawable2,
			int string) {
		if (isShow) {
			if (mTextWarn != null) {
			...
			if (mImageWarn != null) {
				if (drawable != 0) {	
					mImageWarn.setBackground(context.getResources().getDrawable(
							drawable));
			...
			if (mImageWarn2 != null) {
				if (drawable2 != 0) {
					mImageWarn2.setBackground(context.getResources()
							.getDrawable(drawable2));
