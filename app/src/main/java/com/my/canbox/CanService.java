...
public class CanService {
	...
	public static CanService getInstanse(Context context) {
		if (mThis == null) {
	...
	public void canboxDataParser(byte[] data, int len) {
		mHandler.sendMessage(mThis.mHandler.obtainMessage(PARSER_CANBOX_DATA,
				len, 0, data));
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case Canbox.CANBOX_RETURN_AIR:
				if (mAirConditionPanel != null) {
					mAirConditionPanel.postChanged(
							AirConditionPanel.MESSAGE_AIR_CONDITION, msg.arg1,
							msg.arg2, msg.obj);
				}
				break;
			...
			case Canbox.CMD_GROUP_AC:
				if (mAirConditionPanel != null) {
					mAirConditionPanel.postChanged(
							AirConditionPanel.MESSAGE_AIRDATA_TO_ACCONTROL_APK, 0, 0, null);
				}
				break;
			// break;
			...
			case Canbox.CANBOX_DOOR_STATUS:
				if (mDoorStatusPanel != null) {
					mDoorStatusPanel.postChanged(
							DoorStatusPanel.MESSAGE_DOOR_CONDITION, msg.arg1);
				}
				break;
	...
	public static void updateCanboxSettings() {
		if (mThis != null) {
			mThis.mCanbox.updateCanboxSettings();
		}
	}
	...
	public void updateCanbox() {
		...
		if (mCanbox != null) {
			...
			if (GlobalDef.mIs8600 || GlobalDef.mMediaInfoToastBackground != 0) {
				registerListener();
			} else {
				unregisterListener();
			}
	...
	private void initCanboxTime(){
		...
		if (mUpdateCaboxTime != 0) {
			...
			if (mUpdateCaboxTime == 60000) { //1 min
				...
				mHandler.removeMessages(CANBOX_UPDATE_TIME);				
				mHandler.sendEmptyMessageDelayed(CANBOX_UPDATE_TIME,
						second*1000);
			}
			registerReceiver();
		} else {
	...
	private Handler mHandlerMediaInfoToCanbox = new Handler() {
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case 1:
//				if (msg.obj != null) {
				...
				public void onReceive(Context context, Intent intent) {
					...
					// }
						...
						if (CarUtil.getCanboxType() != null) {
							...
							CarUtil.getCanboxInstance().setMediaSrc(
									MyCmd.SOURCE_RADIO, (byte) 1, b);
						}
						...
						if (GlobalDef.mIs8600) {
							...
							if (s != null) {
								GlobalDef.updateLcd(fm | MyCmd.SOURCE_RADIO, s);
							}							
						}
						...
						if (CarUtil.getCanboxType() != null) {
							...
							if (mAppSource == MyCmd.SOURCE_MUSIC) {
								CarUtil.getCanboxInstance().setMediaMoreInfo(
										MyCmd.SOURCE_MUSIC, value1, value3,
										value2, value4);
							}
						}
							...
							if (value3 > 0){
								...
								++value1;
								 s = String.format(Locale.ENGLISH,
											"%03d-%02d%02d", value1, time / 60,
											time % 60);
							} else {
								s = "MP3";
							}
							
							GlobalDef.updateLcd(MyCmd.SOURCE_MUSIC, s);
						...
						if (CarUtil.getCanboxType() != null) {
							...
							if (mAppSource == MyCmd.SOURCE_DVD) {
								CarUtil.getCanboxInstance().setMediaMoreInfo(
										MyCmd.SOURCE_DVD, value1, value3,
										value2, 0);
							}
						}
						...
						if (GlobalDef.mIs8600) {
							...
							String s = String.format(Locale.ENGLISH,
									"%03d-%02d%02d", mTrackCurrent,
									mCurTime / 60, mCurTime % 60);
							GlobalDef.updateLcd(MyCmd.SOURCE_DVD, s);
						...
						if (CarUtil.getCanboxType() != null) {
							...
							if (mAppSource == MyCmd.SOURCE_VIDEO) {
								CarUtil.getCanboxInstance().setMediaMoreInfo(
										MyCmd.SOURCE_VIDEO, value1, value3,
										value2, value4);
							}
						}
						...
						if (GlobalDef.mIs8600) {
							...
							GlobalDef.updateLcd(MyCmd.SOURCE_VIDEO, s);
					...
					else if (MyCmd.BROADCAST_CMD_FROM_MUSIC.equals(action)) 
					{
						if (mCanbox != null || (GlobalDef.mMediaInfoToastBackground != 0)) 
						{
							...
							if (mCanbox != null) {
								...
								mHandlerMediaInfoToCanbox.sendMessageDelayed(
										mHandlerMediaInfoToCanbox
												.obtainMessage(1, artist), 40);
								mHandlerMediaInfoToCanbox.removeMessages(2);
								mHandlerMediaInfoToCanbox.sendMessageDelayed(
										mHandlerMediaInfoToCanbox
												.obtainMessage(2, album), 120);
							}
							if (GlobalDef.mMediaInfoToastBackground != 0) {
						...
						doBTCmd(intent);
							...
							.equals("AUTONAVI_STANDARD_BROADCAST_SEND")) {
...
//								mCanbox.updateCompass(direction);
			...
			if (mCanbox != null
					&& (mCanbox.isSupportCompass() || (CarUtil
							.getExternalRadarId() > 0))) {
...
//		if (GlobalDef.mMediaInfoToastBackground != 0) {
			...
			case MyCmd.Cmd.BT_SEND_A2DP_STATUS: {
				...
				if (GlobalDef.getTestingEx() && mCanbox != null){
...
//		}
	...
	private static final String RADIO_SOURCE_CHANGE = "com.my.radio.SOURCE_CHANGE";
	private static final String DVD_SOURCE_CHANGE = "com.my.dvd.SOURCE_CHANGE";
	private static final String AUDIO_SOURCE_CHANGE = "com.my.audio.SOURCE_CHANGE";
	private static final String VIDEO_SOURCE_CHANGE = "com.my.video.SOURCE_CHANGE";
	private static final String IPOD_SOURCE_CHANGE = "com.my.ipod.SOURCE_CHANGE";
	private static final String MY_OUT_VOLUME_CHANGE = "com.my.out.VOLUME_CHANGE";
	private static final String BT_PHONE_BROADCAST = "com.my.bt.BT_PHONE_BROADCAST";
		...
		if (mContext != null) {
			try {
				...
				if (s != null && s.length() > 0) {
					...
					TextView tv = (TextView) v.findViewById(R.id.mode_text);
	...
	private void showToastMediaBackgroundBTMusic(String s) {
		if (GlobalDef.mMediaInfoToastBackground != 0) {
			...
			if (s != null && !s.equals(mBTMusicName)) {
				...
				mBTMusicName = s;
				if (!(
						(AppConfig.CAR_UI_BT_MUSIC.equals(AppConfig.getTopActivity())) ||
								(AppConfig.CAR_BT.equals(AppConfig.getTopActivity())) 
