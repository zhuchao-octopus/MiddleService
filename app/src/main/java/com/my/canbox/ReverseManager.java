		...
		if (mView == null) {
			...
			mLayoutParams = new WindowManager.LayoutParams(
					...
					LayoutParams.FLAG_LAYOUT_NO_LIMITS
							| LayoutParams.FLAG_LAYOUT_IN_SCREEN | LayoutParams.FLAG_FULLSCREEN,
...
//	private final static int HIDE_EMPTY = 1; 
	...
	private static Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			switch (msg.what) {			
			case START_UI:
//				 Log.d("camera", "START_UI");
				mUI.onResume();
//				if(msg.arg1==0){
////					hideEmpty();
//					mHandler.sendEmptyMessageDelayed(HIDE_EMPTY, 100);
//				}else{
//					mHandler.sendEmptyMessageDelayed(HIDE_EMPTY, msg.arg1);
//				}
				break;
//			case HIDE_EMPTY:
//				hideEmpty();
...
//			}
			...
			if (GlobalDef.mReverseBrightness != 0) {
				Log.d("ReverseManager", "mReverseBrightness:"+GlobalDef.mReverseBrightness);
				Util.setFileValue(GlobalDef.BRIGHTNESS_CVBS,
						GlobalDef.mReverseBrightness);
			}
			if (GlobalDef.mReverseContrast != 0) {
				Log.d("ReverseManager", "mReverseContrast:"+GlobalDef.mReverseContrast);
				Util.setFileValue(GlobalDef.BRIGHTNESS_CONTRAST,
						GlobalDef.mReverseContrast);
			}
			if(mView == null){
				init(context);
			}
...
//			}
			...
			if (Build.VERSION.SDK_INT >= 26 && Build.VERSION.SDK_INT < 30) {
				GlobalDef.sendByCarServiceToSystemUI(context, "com.android.systemui",
...
//					mHandler.sendEmptyMessageDelayed(REMOVE_UI,delay);
			...
			if (GlobalDef.mReverseBrightness != 0) {
				Util.setFileValue(GlobalDef.BRIGHTNESS_CVBS,
						GlobalDef.CVBS_DEFALUT_BRIGHTNESS);
			}

			if (GlobalDef.mReverseContrast != 0) {
				Util.setFileValue(GlobalDef.BRIGHTNESS_CONTRAST,
						GlobalDef.CVBS_DEFALUT_CONTRAST);
			}
