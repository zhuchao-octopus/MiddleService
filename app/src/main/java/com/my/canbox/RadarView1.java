...
public class RadarView1 extends ImageView {
	...
	private byte[] mRadarDataFrontEx = new byte[2];
	private int[] mRadarFrontStartXEx = new int[2];
	// private int[] mRadarFrontStartYEx = new int[2];

	private byte[] mRadarDataFront = new byte[4];
	private byte[] mRadarDataBack = new byte[4];

	private int[] mRadarColorFront = new int[4];
	private int[] mRadarColorBack = new int[4];
	...
	private byte[] mRadarDataLeft = new byte[4];
	private byte[] mRadarDataRight = new byte[4];

	private int[] mRadarColorLeft = new int[4];
	private int[] mRadarColorRight = new int[4];

	private int[] mRadarLeftRightStartY = new int[4];
	...
	protected void onDraw(Canvas canvas) {
		...
		for (i = 0; i < 4; ++i) {
			...
			case 3:
				for (int j = 0; j < mRadarDataFront[i]; ++j) {
					canvas.drawLine(mRadarFrontStartX[i], mRadarFrontStartY[i]
							+ RADAR_BAR_INTERVAL * (RADAR_BAR_NUM - j),
							mRadarFrontStartX[i] + RADAR_BAR_HEIGHT,
							mRadarFrontStartY[i] + RADAR_BAR_INTERVAL
									* (RADAR_BAR_NUM - j), mPaint);
				}
				break;
			}
		}
		...
		for (i = 0; i < 4; ++i) {
			...
			case 3:
				for (int j = 0; j < mRadarDataBack[i]; ++j) {
					canvas.drawLine(mRadarBackStartX[i], mRadarBackStartY[i]
							+ RADAR_BAR_INTERVAL * j, mRadarBackStartX[i]
							+ RADAR_BAR_HEIGHT, mRadarBackStartY[i]
							+ RADAR_BAR_INTERVAL * j, mPaint);
				}
				break;
			}
		}
		...
		for (i = 0; i < 2; ++i) {
			...
			for (int j = 0; j < mRadarDataFrontEx[i]; ++j) {
				...
				canvas.drawLine(mRadarFrontStartXEx[i], yStart + exInterVal
						* (RADAR_BAR_NUM - j), mRadarFrontStartXEx[i]
						+ RADAR_BAR_HEIGHT, yStart + exInterVal
						* (RADAR_BAR_NUM - j), mPaint);
			}
			...
			for (i = 0; i < 4; ++i) {
				...
				case 3:
					for (int j = 0; j < mRadarDataRight[i]; ++j) {
						canvas.drawLine(mRadarRightStartX + exInterVal * j,
								mRadarLeftRightStartY[i], mRadarRightStartX
										+ exInterVal * j,
								mRadarLeftRightStartY[i] + RADAR_BAR_HEIGHT,
			...
			for (i = 0; i < 4; ++i) {
				...
				case 3:
					...
					for (int j = 0; j < mRadarDataLeft[i]; ++j) {
						...
						canvas.drawLine(mRadarLeftStartX + exInterVal
								* (barNum - j), mRadarLeftRightStartY[i],
								mRadarLeftStartX + exInterVal * (barNum - j),
								mRadarLeftRightStartY[i] + RADAR_BAR_HEIGHT,
