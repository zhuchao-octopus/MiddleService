...
public class UpdateRaise extends Canbox {
	...
	public void startConnect() {
		...
		mUpdateDialog.show();
		Util.doSleep(200);
		byte[] data = new byte[] { (byte) 0xD9, (byte) 0x05, (byte) 0x01,
				(byte) 0x5E, (byte) 0x6A, (byte) 0xEA, (byte) 0x87 };
		mSendLen = -1;
...
//				sendDataToCanbox(data, data.length);
	...
	public void parseCanboxData(byte[] data, int len) {
		...
		case (byte) 0x69:
			switch (data[2]) {
			case 1:
				if (data[3] == (byte) 0x5e && data[4] == (byte) 0x6a
						&& data[5] == (byte) 0xea && data[6] == (byte) 0x87) {					
					startUpdate();		
					mUpdateDialog.setTitle("raise " + mContext.getString(R.string.updateing_canbox));
				}
				break;
			case (byte) 0x2:
				continuUpdate(data[3] & 0xff, ((data[4] & 0xff) << 16)
						| ((data[5] & 0xff) << 8) | ((data[6] & 0xff) << 0));
				break;
			case (byte) 0x81:
			...
			case (byte) 0x83:// stop
				...
				break;
			case (byte)0x84:// stop
				mUpdateDialog.setMsg(mContext
						.getString(R.string.update_canbox_fail)
						+ (String.format("%02d_%02d_%02d", (data[3] & 0xff),
