...
public class DaoQiDaoJun extends Canbox {
	...
	private final static byte KEYS_WHEEL[][] = {
			...
			{ 0x5, MyCmd.Keycode.MODLE }, { 0x6, MyCmd.Keycode.SPEECH },
	...
	private final static byte KEYS_WHEEL2[][] = { { 0x1, MyCmd.Keycode.POWER },
			...
			{ 0x18, MyCmd.Keycode.VOLUME_ROLL_DOWN },
			{ 0x19, MyCmd.Keycode.ROLL_NEXT },
			{ 0x1a, MyCmd.Keycode.ROLL_PREV }, };

	private byte[] getCarTypeCmd() {
		byte[] cmd = new byte[] { (byte) 0x72, 0x15, 0x14, 0x1, 0x1, 0x1, 0x3,
				(byte) 0xaf, (byte) 0xfb, (byte) 0xfb, (byte) 0xfb, 0, 0, 0, 0,
				0, 0, 0, 0 };
	...
	public void parseACInfo(byte[] data) {
		...
		airData[0] = (byte) (((data[2] & 0x10) >> 4) | ((data[3] & 0x10) >> 4)
				| ((data[2] & 0x60) << 0) | ((data[2] & 0x80) << 0)
				| ((data[3] & 0x20) >> 3) | ((data[8] & 0x04) << 2));
	...
	public void sendDataToCanbox(byte[] data, int len) { // default is simple
		...
		Log.d("DaoQiDaoJun",
				"sendDataToCanbox buf:"
						+ Util.byte2HexStr(send));
		sendCanboxInfo(send);
	}
			...
			public void run() {
				...
				try {
					...
					ComponentName componentName = new ComponentName(
							"com.dogen.usbcameradogen",
							"com.dogen.usbcameradogen.services.DogenServiceManage");
					intent.setComponent(componentName);
					
					Class<?>[] paramClasses = {};
					Method getVolumeList = Context.class.getMethod(
							"startForegroundService", paramClasses);
			...
			mBroadcastReceiver = new BroadcastReceiver() {
				...
				public void onReceive(Context context, Intent intent) {
					...
					if (intent.getAction().equals(MSG_CAN_SEND_DATA_ALL_INFO)) {
						...
						if (buf != null) {
							Log.d("DaoQiDaoJun",
									"MSG_CAN_SEND_DATA_ALL_INFO buf:"
