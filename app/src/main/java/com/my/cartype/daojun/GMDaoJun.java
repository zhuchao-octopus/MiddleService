...
public class GMDaoJun extends Canbox {
	...
	private byte mKeyPannel[][]  = KEYS_PANNEL_NORMAL;
	private final static byte KEYS_PANNEL_GL8[][] = {
			{ 0x1, KEY_PREVIOUSSONG }, { 0x2, KEY_NUM_5 }, { 0x3, KEY_NUM_4 },
			{ 0x4, KEY_NUM_3 }, { 0x5, KEY_POWER }, { 0x6, KEY_GPS },
			{ 0x7, KEY_FM }, { 0x8, KEY_NUM_1 }, { 0x9, KEY_NUM_2 },
			{ 0xa, KEY_NEXTSONG }, { 0xb, KEY_MENU }, { 0xc, KEY_BACK },
			{ 0xd, KEY_EQ }, { 0xe, KEY_EJECT }, { 0x11, KEY_SET },
			{ 0x12, KEY_MEDIA }, { 0x13, KEY_PLAYPAUSE }, { 0x14, KEY_FM },
			{ 0x15, KEY_MUTE }, { 0x16, KEY_NUM_6 },
			{ 0x17, AK_KEYPAD_VOLUME_A }, { 0x18, AK_KEYPAD_VOLUME_D },
			{ 0x19, KEY_SEEK_NEXT }, { 0x1a, KEY_SEEK_PREV },
			{ 0x34, MyCmd.Keycode.KEY_TURN_A },
			{ 0x35, MyCmd.Keycode.KEY_TURN_D }, 
			};

	private final static byte KEYS_PANNEL_ASTRA_J[][] = {

	{ 0x17, AK_KEYPAD_VOLUME_A }, { 0x18, AK_KEYPAD_VOLUME_D },
			{ 0x9, KEY_PREVIOUSSONG }, { 0x6, KEY_NEXTSONG },
			{ 0xb, KEY_POWER }, { 0xc, KEY_NUM_1 }, { 0xd, KEY_NUM_2 },
			{ 0xe, KEY_NUM_3 }, { 0xf, KEY_NUM_4 }, { 0x1e, KEY_NUM_5 },
			{ 0x1f, KEY_NUM_6 }, { 0x1, KEY_HOME }, { 0x16, KEY_MODE },
			{ 0x1b, KEY_PLAYPAUSE }, { 0x12, KEY_BT }, { 0x50, KEY_EQ },
			{ 0x19, MyCmd.Keycode.KEY_TURN_A },
			{ 0x1a, MyCmd.Keycode.KEY_TURN_D }, { 0x3, KEY_MENU },
			{ 0x14, KEY_SET }, { 0x4, KEY_GPS }, { 0x5, KEY_FM },
			{ 0xa, MyCmd.Keycode.AS }, { 0x11, KEY_EJECT }, };

	private final static byte KEYS_PANNEL_ENVISION_L[][] = {
			{ 0x1, KEY_NUM_2 }, { 0x2, KEY_MUTE }, { 0x3, KEY_BACK },
			{ 0x4, KEY_GPS }, { 0x5, KEY_FM }, { 0x6, KEY_PREVIOUSSONG },
			{ 0x7, KEY_POWER }, { 0x8, KEY_MENU }, { 0x9, KEY_GPS },
			{ 0xa, KEY_NUM_3 }, { 0xb, KEY_NUM_4 }, { 0xc, KEY_NUM_5 },
			{ 0xd, KEY_NUM_6 }, { 0x10, MyCmd.Keycode.AS },
			{ 0x11, KEY_EJECT }, { 0x12, KEY_SET },
			{ 0x13, MyCmd.Keycode.TIME_SETTING }, { 0x14, KEY_EQ },
			{ 0x15, KEY_NUM_1 }, { 0x16, KEY_NEXTSONG },
			{ 0x17, AK_KEYPAD_VOLUME_A }, { 0x18, AK_KEYPAD_VOLUME_D },
			{ 0x19, KEY_SEEK_NEXT }, { 0x1a, KEY_SEEK_PREV },
			{ 0x1b, KEY_PLAYPAUSE }, { 0x40, KEY_AUX }, { 0x36, KEY_DVD }, };

	private final static byte KEYS_PANNEL_NORMAL[][] = { { 0x1, KEY_POWER },
			{ 0x2, KEY_PREVIOUSSONG }, { 0x3, KEY_NEXTSONG }, { 0x4, KEY_SET },
			{ 0x5, KEY_EQ }, { 0x6, KEY_BACK }, { 0x7, KEY_FM },
			{ 0x8, KEY_DVD }, { 0x9, KEY_MUTE }, { 0xa, KEY_NUM_1 },
			{ 0xb, KEY_NUM_2 }, { 0xc, KEY_NUM_3 }, { 0xd, KEY_NUM_4 },
			{ 0xe, KEY_NUM_5 }, { 0xf, KEY_NUM_6 }, { 0x10, KEY_GPS },
			{ 0x11, KEY_EJECT }, { 0x12, KEY_GPS },
			{ 0x13, MyCmd.Keycode.TIME_SETTING }, { 0x14, KEY_FM },
			{ 0x15, MyCmd.Keycode.AS }, { 0x16, KEY_MENU },
			{ 0x17, AK_KEYPAD_VOLUME_A }, { 0x18, AK_KEYPAD_VOLUME_D },
			{ 0x19, KEY_SEEK_NEXT }, { 0x1a, KEY_SEEK_PREV },
			{ 0x1b, KEY_PLAYPAUSE }, { 0x1c, KEY_PREVIOUSSONG },
			{ 0x1d, KEY_NEXTSONG }, { 0x40, KEY_AUX },
			{ 0x34, MyCmd.Keycode.KEY_TURN_A },
			{ 0x35, MyCmd.Keycode.KEY_TURN_D }, { 0x50, KEY_HOME },
			{ 0x51, KEY_SOURCE }, { 0x52, KEY_PLAYPAUSE }, { 0x53, KEY_MENU },
			{ 0x54, KEY_MEDIA }, { 0x55, KEY_GPS }, { 0x56, KEY_GPS },
			{ 0x57, KEY_GPS }, { 0x58, KEY_SEEK_PREV },
			{ 0x59, KEY_SEEK_NEXT }, { 0x5a, KEY_PREVIOUSSONG },
			{ 0x5b, KEY_NEXTSONG }, };
	...
	private void parseACInfo(byte[] data, int len) {
		...
		if (((data[3] & 0xf) == 6) || ((data[3] & 0xf) == 7)
				|| ((data[3] & 0xf) == 8) || ((data[3] & 0xf) == 9)) {
			airData[1] |= 0x80;
		}

		if (((data[3] & 0xf) == 4) || ((data[3] & 0xf) == 5)
				|| ((data[3] & 0xf) == 6) || ((data[3] & 0xf) == 9)) {
			airData[1] |= 0x40;
		}

		if (((data[3] & 0xf) == 3) || ((data[3] & 0xf) == 4)
				|| ((data[3] & 0xf) == 8) || ((data[3] & 0xf) == 9)) {
			airData[1] |= 0x20;
		}
		...
		case 0x9:
			...
			if (data[2] != data0x9[2]) {
				if (data[2] == 0x1 || data[2] == 0x2 || data[2] == 0x3 || data[2] == 0x4) {
					...
					if (top != null
							&& top.contains("com.canboxsetting.AnStartActivity")) {
						topIsCamera = true;
					}
...
//			sendCanboxInfo("com.canboxsetting", data);
	...
	private byte[] data0x8 = new byte[12];
	private byte[] data0x9 = new byte[3];
