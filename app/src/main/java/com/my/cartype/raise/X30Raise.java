...
public class X30Raise extends Canbox {
	...
	private final static byte KEYS_WHEEL[][] = { { 0x1, AK_KEYPAD_VOLUME_A },
			...
			{ 0x6, KEY_MUTE }, { 0x7, KEY_SOURCE }, { 0x9, KEY_BT_DIAL },
			{ 0xA, KEY_BT_HANG },
	...
	private final static byte KEYS_WHEEL2[][] = { { 0x1, KEY_POWER },
			{ 0x2, AK_KEYPAD_VOLUME_A }, { 0x3, AK_KEYPAD_VOLUME_D },
			{ 0x4, KEY_MUTE }, { 0x5, MyCmd.Keycode.AS },
			{ 0x6, MyCmd.Keycode.KEY_SEEK_PREV },
			{ 0x7, MyCmd.Keycode.KEY_SEEK_NEXT },
			{ 0x8, MyCmd.Keycode.PLAY_PAUSE },
			{ 0x9, MyCmd.Keycode.KEY_TURN_A },
			{ 0xa, MyCmd.Keycode.KEY_TURN_D },

			{ 0xa, MyCmd.Keycode.NUMBER1 }, { 0xb, MyCmd.Keycode.NUMBER2 },
			{ 0xc, MyCmd.Keycode.NUMBER3 }, { 0xd, MyCmd.Keycode.NUMBER4 },
			{ 0xe, MyCmd.Keycode.NUMBER5 }, { 0xf, MyCmd.Keycode.NUMBER6 },
			{ 0x10, MyCmd.Keycode.NUMBER1 },

			{ 0x11, MyCmd.Keycode.KEY_SEEK_PREV }, { 0x12, MyCmd.Keycode.KEY_SEEK_NEXT },

			{ 0x13, KEY_FM }, { 0x14, KEY_MEDIA }, { 0x15, MyCmd.Keycode.KEY_SHUFFLE },
			{ 0x16, MyCmd.Keycode.KEY_REPEAT },

			{ 0x17, KEY_GPS }, { 0x18, KEY_MEDIA }, { 0x19, KEY_MIC },
			{ 0x1a, KEY_BT }, { 0x1b, MyCmd.Keycode.KEY_RADIO_SCAN },
			{ 0x1c, MyCmd.Keycode.SCREEN_BRIGHTNESS },

			{ 0x1d, MyCmd.Keycode.BT_DIAL }, { 0x1e, MyCmd.Keycode.BT_HANG },
			{ 0x1F, KEY_BACK }, { 0x20, KEY_HOME }, { 0x21, KEY_HOME },

			{ 0x22, KEY_MEDIA },

	};
	
	private final static byte KEYS_WHEEL2_B50[][] = { { 0x1, KEY_POWER },
		{ 0x2, AK_KEYPAD_VOLUME_A }, { 0x3, AK_KEYPAD_VOLUME_D },
		{ 0x4, KEY_MUTE }, { 0x5, KEY_BACK },
		{ 0x6, MyCmd.Keycode.KEY_SEEK_PREV },
		{ 0x7, MyCmd.Keycode.KEY_SEEK_NEXT },
		{ 0x8, MyCmd.Keycode.PLAY_PAUSE },
		{ 0x9, MyCmd.Keycode.KEY_TURN_A },
		...
		{ 0x13, KEY_FM }, { 0x14, KEY_BT }, { 0x15, KEY_MODE },
		{ 0x16, KEY_MIC },
	...
	public void setPhone(int phone_status, String num) {// default is simple box
		...
		if (mPhoneStatus > HFP_INFO_CONNECTED
				&& phone_status == HFP_INFO_CONNECTED) {
			if (mData != null) {
