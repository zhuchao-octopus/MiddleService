...
public class DongFengRaise extends Canbox {
	...
	public DongFengRaise() {
		...
		if (CarUtil.getModelId() == 0 || CarUtil.getModelId() == 1
				|| CarUtil.getModelId() == 15 || CarUtil.getModelId() == 8) {
	...
	private final static byte KEYS_WHEEL[][] = {
			...
			{ 0xb, MyCmd.Keycode.KEY_SEEK_PREV },
			{ 0xc, MyCmd.Keycode.KEY_SEEK_NEXT },
			{ 0x32, MyCmd.Keycode.NAVIGATION },
			{ (byte) 0x80, MyCmd.Keycode.RADIO },
			{ (byte) 0x81, MyCmd.Keycode.HOME },
			{ (byte) 0x82, MyCmd.Keycode.BACK },
			{ (byte) 0x83, MyCmd.Keycode.AUDIO },
			{ (byte) 0x84, MyCmd.Keycode.KEY_DISPLAY },
			{ (byte) 0x85, MyCmd.Keycode.PREVIOUS },
			{ (byte) 0x86, MyCmd.Keycode.KEY_FM },
			{ (byte) 0x87, MyCmd.Keycode.POWER },
			{ (byte) 0x88, MyCmd.Keycode.NEXT },
			{ (byte) 0x89, MyCmd.Keycode.KEY_AM },
			{ (byte) 0x8a, MyCmd.Keycode.VIDEO },
			{ (byte) 0x8b, MyCmd.Keycode.BT },
			{ (byte) 0x8c, MyCmd.Keycode.SETUP },
			{ (byte) 0x8d, MyCmd.Keycode.EQ },
			{ (byte) 0x8e, MyCmd.Keycode.KEY_360 },
			{ (byte) 0x8f, MyCmd.Keycode.AS },
			{ (byte) 0x90, MyCmd.Keycode.AUDIO },
			{ (byte) 0x91, MyCmd.Keycode.MENU },
			{ (byte) 0x92, MyCmd.Keycode.NEXT },
			{ (byte) 0x93, MyCmd.Keycode.PREVIOUS },
			{ (byte) 0x94, MyCmd.Keycode.KEY_SEEK_NEXT },
			{ (byte) 0x95, MyCmd.Keycode.KEY_SEEK_PREV },
			{ (byte) 0x96, MyCmd.Keycode.PLAY_PAUSE },
			{ (byte) 0x97, MyCmd.Keycode.MUTE },
			{ (byte) 0x98, MyCmd.Keycode.EQ },
			{ (byte) 0x99, MyCmd.Keycode.PLAY_PAUSE },
