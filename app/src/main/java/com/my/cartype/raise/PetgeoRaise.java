...
import com.my.canbox.AutoParkingMsgManager;
...
public class PetgeoRaise extends Canbox {
	...
	{ 0x4, KEY_NEXTSONG },
			...
			{ (byte) 0x17, KEY_PREVIOUSSONG },
			{ (byte) 0x18, KEY_NEXTSONG },
			{ 0x19, AK_KEYPAD_VOLUME_A }, 
			{ 0x1a, AK_KEYPAD_VOLUME_D },
			{ 0x22, MyCmd.Keycode.KEY_MEM_INFO }, 
			...
			{ (byte) 0x97, KEY_PREVIOUSSONG },
			{ (byte) 0x98, KEY_NEXTSONG },
			{ (byte) 0x99, KEY_FM },
			{ (byte) 0x9a, MyCmd.Keycode.BRIGHTNESS },
			{ (byte) 0x9b, KEY_HOME },
			{ (byte) 0x9c, KEY_PREVIOUSSONG },
			{ (byte) 0x9d, KEY_NEXTSONG },
			{ (byte) 0x9e, KEY_MENU },
			{ (byte) 0x9f, MyCmd.Keycode.MODLE },

			{ (byte) 0xa0, MyCmd.Keycode.MULT_SOURCE_AND_BT },
			{ (byte) 0xa1, KEY_MEDIA },
			{ (byte) 0xa2, KEY_PLAYPAUSE },
			{ (byte) 0xa3, KEY_BACK },
			{ (byte) 0xa4, KEY_MUTE },
			// { (byte)0xa5, KEY_MODE },
			{ (byte) 0xa6, KEY_HOME },

			{ (byte) 0xb0, KEY_BT_DIAL }, 
			{ (byte) 0xb1, KEY_BT_HANG },
			{ (byte) 0xb2, KEY_BACK }, 
			{ (byte) 0xb3, KEY_GPS },
			{ (byte) 0xb4, MyCmd.Keycode.RDS_TA_SWITCH },
			{ (byte) 0xb5, KEY_FM },
			{ (byte) 0xb6, KEY_MEDIA },
			{ (byte) 0xb7, KEY_SET },
			{ (byte) 0xb8, MyCmd.Keycode.KEY_RECENT_APPS },
			{ (byte) 0xb9, KEY_PREVIOUSSONG }, 
			{ (byte) 0xba, KEY_NEXTSONG },
			{ (byte) 0xbb, KEY_SEEK_PREV }, 
			{ (byte) 0xbc, KEY_SEEK_NEXT },
			{ (byte) 0xbd, KEY_PLAYPAUSE },
			{ (byte) 0xbe, MyCmd.Keycode.ROLL_NEXT },
			{ (byte) 0xbf, MyCmd.Keycode.ROLL_PREV },
			...
			{ (byte) 0x29, MyCmd.Keycode.SPEECH },
			{ (byte) 0x2a, MyCmd.Keycode.KEY_CAR_SETTING },
			{ (byte) 0x30, MyCmd.Keycode.MULT_SOURCE_AND_BT },
			{ (byte) 0x33, MyCmd.Keycode.RADIO },
			{ (byte) 0x2b, MyCmd.Keycode.NAVIGATION },
			{ (byte) 0x2c, MyCmd.Keycode.RADIO },
			{ (byte) 0x2d, MyCmd.Keycode.BT },
			{ (byte) 0x2e, MyCmd.Keycode.SETUP },
			{ (byte) 0x2f, MyCmd.Keycode.KEY_AIR_CONTROL },
			{ (byte) 0x32, MyCmd.Keycode.NAVIGATION },
			{ (byte) 0x35, MyCmd.Keycode.NAVIGATION },

			{ (byte) 0x34, MyCmd.Keycode.SETUP },

			{ (byte) 0x36, MyCmd.Keycode.AUDIO },

			{ (byte) 0x38, KEY_PREVIOUSSONG },
			{ (byte) 0x39, KEY_NEXTSONG },

			{ (byte) 0x40, KEY_PREVIOUSSONG },
			{ (byte) 0x41, KEY_NEXTSONG },
			{ (byte) 0x42, KEY_PREVIOUSSONG },
			{ (byte) 0x43, KEY_NEXTSONG },
			

			{ (byte) 0x58, KEY_PREVIOUSSONG },
			{ (byte) 0x59, KEY_NEXTSONG },
			{ (byte) 0x57, KEY_PREVIOUSSONG },
			{ (byte) 0x56, KEY_NEXTSONG },
			{ (byte) 0x5c, KEY_PLAYPAUSE },
			...
			{ 0x51, MyCmd.Keycode.SETUP },
			{ 0x61, MyCmd.Keycode.BACK },
...
//					checkHideRadar();
		...
		case 0x71: {
			...
			mVersionEx = data[2] + " " + data[3] + " "+ date + "v" + data[8]+ ""
					+ data[9] + ""+ data[10];
//			Log.d("ff", ""+mVersion);
			break;
		}
	...
	public void updateCanboxSettings() {
		...
		showWarningMsg = Settings.System.getInt(mContext.getContentResolver(),
...
//		++play;
		...
		data = new byte[] { 0x9, (byte) 0xc0, s, 1,
