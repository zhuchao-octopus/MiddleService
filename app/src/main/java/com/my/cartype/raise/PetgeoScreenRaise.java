...
public class PetgeoScreenRaise extends Canbox {
	...
	private final static byte KEYS_WHEEL[][] = {		
		...
		{ 0x33, KEY_FM },
		{ 0x34, KEY_SET },
		{ 0x35, MyCmd.Keycode.NAVIGATION },
		{ 0x36, MyCmd.Keycode.AUDIO },
...
//		byteArrayCopy(d, data, 0, 1, len);
		...
		case 0x8: 
		{
			...
			mRadar[6] = getRadarData(data[3]);
			mRadar[7] = getRadarData(data[4]);

			mRadar[0] = getRadarData(data[5]);
			mRadar[1] = getRadarData(data[6]);
			mRadar[2] = getRadarData(data[6]);
			mRadar[3] = getRadarData(data[7]);
