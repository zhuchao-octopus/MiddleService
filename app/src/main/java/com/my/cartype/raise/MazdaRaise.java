...
public class MazdaRaise extends Canbox {
	...
	private final static byte KEYS_PANNEL[][] = { 
		...
		{ 0x3, AK_KEYPAD_VOLUME_A },
			{ 0x2, AK_KEYPAD_VOLUME_D }, 
			{ 0x4, KEY_MUTE },

			{ 0x5, MyCmd.Keycode.HOME },
			{ 0x6, MyCmd.Keycode.BACK },
			{ 0x7, MyCmd.Keycode.RADIO },
			{ 0x8, MyCmd.Keycode.AUDIO },
			...
			{ 0xd, MyCmd.Keycode.KEY_TURN_D },
			{ 0xe, MyCmd.Keycode.KEY_TURN_A },
	...
	private boolean isOneKey(byte b) {
		if (((b & 0xff) == 0x9) || ((b & 0xff) == 0xa)
				|| ((b & 0xff) == 0xb) || ((b & 0xff) == 0xc)
				|| ((b & 0xff) == 0xd) || ((b & 0xff) == 0xe)
				|| ((b & 0xff) == 0x2) || ((b & 0xff) == 0x3)) {
			return true;
		}
...
//		sendDataToCanbox(data, data.length);
	...
	public void parseCanboxData(byte[] data, int len) {
		...
		case 0x25:
			sendTouch(((data[2] & 0xff) << 8) | (data[3] & 0xff),
