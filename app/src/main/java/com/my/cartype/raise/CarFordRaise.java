...
public class CarFordRaise extends Canbox {
	...
	private final static byte KEYS_WHEEL_NORMAL[][] = {
		...
		{ 0x16, MyCmd.Keycode.KEY_AIR_CONTROL },
		{ 0x17, MyCmd.Keycode.KEYAMS_RPT },
	...
	{ 0x20, KEY_NUM_0 }, { 0x21, KEY_NUM_1 }, { 0x22, KEY_NUM_2 },
			...
			{ (byte) 0x5c, AK_KEYPAD_VOLUME_A },
			{ (byte) 0x5d, AK_KEYPAD_VOLUME_D },
			{ (byte) 0x5e, MyCmd.Keycode.KEY_TURN_A },
			{ (byte) 0x5f, MyCmd.Keycode.KEY_TURN_D },

			{ 0x66, MyCmd.Keycode.PLAY_PAUSE },
			{ 0x67, MyCmd.Keycode.AUDIO },
			{ 0x68, MyCmd.Keycode.SETUP},
			...
			{ (byte) 0x70, MyCmd.Keycode.KEY_TURN_A },
			{ (byte) 0x71, MyCmd.Keycode.KEY_TURN_D },
...
//			break;
		...
		case 0x24: {
			...
			door = (((door & 0x40) >> 6) | ((door & 0x80) >> 6)
					| ((door & 0x10) >> 2) | ((door & 0x20) >> 2) | ((door & 0x08) << 1));
		...
		case 0x79: {
			if (!"com.canboxsetting/com.focussync.MainActivity".equals(AppConfig
					.getTopActivity())) {
				if (data[2] >= 1 && data[2] <= 4) {
					try {
						Intent it = new Intent(Intent.ACTION_VIEW);
						it.setClassName("com.canboxsetting",
								"com.focussync.MainActivity");
...
//			string1 = R.string.prompt20;
		...
		if (string != 0) {
			...
			AutoParkingMsgManager.updateView(mContext, drawable1, string,
					string1);
		} else {
			AutoParkingMsgManager.stop();
		}
	...
	public void setMediaMoreInfo(int source, int play, int total, int time,
		...
		byte[] data = new byte[] { (byte) 0xc0, 0x8, 2, (byte) ((play) & 0xFF),
				(byte) (((play) & 0xFF00) >> 8), (byte) (total & 0xFF),
				(byte) (((total) & 0xFF00) >> 8), h, min, sec };
	...
	private void doSyncControl(byte[] data) {
		...
		case 0x20: {
			...
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					...
					data[5] & 0xff, data[4] & 0xff);
			break;
		}
		case 0x22:
			if (mBaud == 0) {
				BroadcastUtil.sendToCarServiceMcuRadio(mContext,
						ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 0x3,
						data[3] - 1);
			}
			break;
		case 0x23:
...
//	}
	...
	private void sendCarType2() {
		...
		if (type >= 1 && type <= 7) {
			...
			byte[] data = new byte[] { (byte) 0x91, 0x2, (byte) type,
					(byte) level };
			sendDataToCanbox(data, data.length);
		}
	}
