...
import com.my.cartype.update.UpdateHiWorld;
import com.my.cartype.update.UpdateLuZheng;
import com.my.cartype.update.UpdateRaise;
import com.my.cartype.update.UpdateSimple;
...
public class CarUtil {
			...
			if (mMcuManager != null) {
				...
				if (mPreCanboxBrake != brake) {
					...
					if (GlobalDef.getContext() != null) {
						BroadcastUtil.sendByCarService(GlobalDef.getContext(),MyCmd.Cmd.MCU_BRAK_CAR_STATUS, brake);
					}
	...
	public static int isNeedSendTime() {
		if (mCanboxType != null) {
			...
			if (mCanboxType.equals(MachineConfig.VALUE_CANBOX_HY)
					|| mCanboxType.equals(MachineConfig.VALUE_CANBOX_RAM_FIAT)
					|| mCanboxType
					...
					|| mCanboxType.equals(MachineConfig.VALUE_CANBOX_HAFER_H2)
					|| mCanboxType
	...
	public void canboxParser(byte[] data, int len) {
		try {
			...
			if (mCanbox != null) {
				...
				if ((GlobalDef.getContext() != null) && isDataDistribution()) {
					Intent i = new Intent(MyCmd.BROADCAST_SEND_FROM_CAN);
					i.putExtra("buf", data);
					GlobalDef.getContext().sendBroadcast(i);
				}
			} else if (false){
				//no used now
				if (((data[0]&0xff) == 0x2e) &&
	...
	public static boolean isHideAirCondition() {
		if (mProVersion >= 3) {
			if ((mAirCondition & AC_CONFIG_HIDE) != 0){
				return true;
			}
		} else {
			if (mAirCondition == 2){
	...
	public static boolean isChangeAirCondition() {
		if (mProVersion >= 3) {
			if ((mAirCondition & AC_CONFIG_TEMP_CHANGE) != 0){
				return true;
			}
		} else {
			if (mAirCondition == 1){
	...
	public static boolean isHideOurDoorTemp() {
		if (mProVersion >= 3) {
			if ((mAirCondition & AC_CONFIG_OURDOOR_HIDE) != 0){
				return true;
			}
		} 
	...
	private String getCanboxSetting() {
		...
		if (mCanboxType == null) {
			mCanboxType = MachineConfig
					.getPropertyReadOnly(MachineConfig.KEY_CAN_BOX);
		}
		if (mCanboxType != null) {
				...
				for (int i = 1; i < ss.length; ++i) {
					if (ss[i]
						...
						mCarType2 = Integer.valueOf(ss[i].substring(1));
							...
							.startsWith(MachineConfig.KEY_SUB_CANBOX_EQ)) {
						mCarEQ = Integer.valueOf(ss[i].substring(1));
					} else if (ss[i]
							.startsWith(MachineConfig.KEY_SUB_CANBOX_OTHER)) {
						...
						mProIndex = Integer.valueOf(ss[i].substring(1));
							...
							.startsWith(MachineConfig.KEY_SUB_CANBOX_UPDATE_TIME)) {
						mUpdateTime = 0;
						...
						if (mProId != null && mProId.length() >= 4) {
							...
							if (mProId.contains("-")) {
								...
								if ((mProId.length() - start) == 2) {
									...
									mCateId = Integer.valueOf(mProId
			...
			String appHide = MachineConfig
					.getPropertyOnce(MachineConfig.KEY_APP_HIDE);
			if (appShow == null || !appShow.contains(AppConfig.HIDE_CANBOX_AC)
					|| (appHide != null && appHide.contains(AppConfig.HIDE_CANBOX_AC))) {
				mIsShowAC = false;
			} else {
				mIsShowAC = true;
			}
			
			if (appShow == null || !appShow.contains(AppConfig.HIDE_CANBOX_EQ)
					|| (appHide != null && appHide.contains(AppConfig.HIDE_CANBOX_EQ))) {
				mIsShowEQ = false;
			} else {
				mIsShowEQ = true;
			}
	...
	public static boolean getRadarBeep() {
		if ((mOtherSettings & MachineConfig.VALUE_CANBOX_OTHER_RADAR_BEEP) != 0) {
			return true;
		}
		return false;
	}
	...
	public static boolean getHideRadarUI() {
		if ((mOtherSettings & MachineConfig.VALUE_CANBOX_OTHER_RADAR_UI) != 0) {
			return true;
		}
		return false;
	}
	...
	public static boolean getShowRadarUIOnlyInReverse() {
		if ((mOtherSettings & MachineConfig.VALUE_CANBOX_OTHER_RADAR_UI_ONLY_IN_REVERSE) != 0) {
			return true;
		}
		return false;
	}
	...
	public static boolean getRightCameraExist() {
		if ((mOtherSettings & MachineConfig.VALUE_CANBOX_OTHER_RIGHT_CAMERA) != 0) {
			return true;
		}
		return false;
	}
	...
	public static boolean isDataDistribution() {
		if ((mOtherSettings & MachineConfig.VALUE_CANBOX_OTHER_DATA_DISTRIBUTION) != 0) {
			return true;
		}
		return false;
	}
	...
	public static void updateCanbox(String f, Context c,String canType) {
		if (mCarUtil != null) {
			...
			if (canType == null){
				Toast.makeText(c, "no canbox manufacturer update fail!",
...
//		return false;
	...
	public static boolean isFocusSync3Reverse() {
		if (getCanboxType() != null
				&& (getCanboxType().equals(
						MachineConfig.VALUE_CANBOX_FORD_SIMPLE) || getCanboxType()
						.equals(MachineConfig.VALUE_CANBOX_FORD_EXPLORER_SIMPLE))) {
			if (getCarType() == 3) {
				return true;
			}
