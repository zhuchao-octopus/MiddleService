...
public class ChryslerSimple extends Canbox {
	...
	public void setContext(Context c) {
		...
		int volume = MachineConfig.getIntProperty2(
	...
	private final static byte KEYS_WHEEL[][] = { { 0x1, AK_KEYPAD_VOLUME_A },
			{ 0x2, AK_KEYPAD_VOLUME_D }, { 0x4, KEY_PREVIOUSSONG },
			{ 0x3, KEY_NEXTSONG },

			{ 0x5, KEY_MUTE },

			{ 0x6, KEY_BT }, { 0x7, KEY_SOURCE }, { 0x8, KEY_MIC },
			{ 0x9, KEY_BT }, };
	...
	public void sendEqToCanbox(byte[] eq) {
		if (eq != null && eq.length >= 11) {
			...
			eq2[2] = (byte) eqC(eq[2] + eq[3] + eq[4]);
			eq2[3] = (byte) eqC(eq[5] + eq[6] + eq[7]);
			eq2[4] = (byte) eqC(eq[8] + eq[9] + eq[10]);
	...
	public void setMediaMoreInfo(int source, int play, int total, int time,
		...
		byte[] data;
		data = new byte[] { (byte) 0xc0, 0x8, 0x10, 0, 0, (byte) play, 0, 0, 0,
				0 };
		sendDataToCanbox(data, data.length);
	...
	public void setMediaSrc(int source, byte type, byte[] b) {
		...
		byte[] data = new byte[] { (byte) 0xc0, 0x8, 0x1, 0x1, b[0], b[1],
				b[2], b[3], 0, 0 };
		sendDataToCanbox(data, data.length);
	}
