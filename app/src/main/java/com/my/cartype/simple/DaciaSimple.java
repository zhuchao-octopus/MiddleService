...
//			{ 0x6, KEY_MUTE }, 
			...
			{ 0x16, MyCmd.Keycode.MULT_NEXT_AND_HANG },
			{ 6, MyCmd.Keycode.MULT_PREV_AND_RECEIVE },
	...
	private void parseACInfo(byte[] data, int len) {
		...
		if (airData[0] != mAirData[0] || airData[1] != mAirData[1]
				|| airData[2] != mAirData[2] || airData[3] != mAirData[3]
				|| airData[4] != mAirData[4] || airData[5] != mAirData[5]
				|| airData[6] != mAirData[6] || airData[7] != mAirData[7]) {
			airControl = true;
...
//				}
	...
	private void do360CameraSwitch(int s, int data) {
		...
		if (top != null
				&& top.contains("com.my.frontcamera.BackCameraActivity")) {
			topIsCamera = true;
		}
		if (s == 0) {
			if (topIsCamera){
