...
public class FordExplorerSimple extends Canbox {
	...
	{ 0x20, KEY_NUM_0 }, { 0x21, KEY_NUM_1 }, { 0x22, KEY_NUM_2 },
			...
			{ 0x33, KEY_FM }, { 0x34, KEY_FM }, { 0x35, KEY_DVD },
			{ 0x36, KEY_AUX },
			{ 0x37, KEY_HOME },
			{ 0x38, KEY_EQ },
			{ 0x39, KEY_BT },
			...
			{ 0x48, KEY_PLAYPAUSE },
			{ 0x4a, MyCmd.Keycode.KEY_SEEK_NEXT },
			{ 0x49, MyCmd.Keycode.KEY_SEEK_PREV },
			...
			// { 0x5e, 0 },
			// { 0x5f, 0 },
			{ 0x62, MyCmd.Keycode.PLAY_PAUSE }, { 0x68, MyCmd.Keycode.BACK },
			{ 0x6A, MyCmd.Keycode.SETUP }, { 0x6F, MyCmd.Keycode.AUDIO },
	...
	private void parseWheelKey(byte[] data, int len) {
		...
		default:
			...
			if (key != 0) {
				doKey(key, data[3]);
				if (data[2] == 0x60 || data[2] == 0x61
						|| data[2] == (byte) 0xf1 || data[2] == (byte) 0xf0) {
					Util.doSleep(1);
					doKey(0, 0);
				}
			} else {
	...
	private void parseACInfo(byte[] data, int len) {
		...
		if (outDoorTemp) {
			...
			GlobalDef.sendByCarServiceToSystemUI(mContext,
					"com.android.systemui", MyCmd.Cmd.SET_OUT_DOOR_TEMP, "" + t
							+ unit);
	...
	public void parseCanboxData(byte[] data, int len) {
		...
		case 0x24: {
			...
			door = (((door & 0x4) << 3) | ((door & 0x40) >> 6)
					| ((door & 0x80) >> 6) | ((door & 0x10) >> 2)
					| ((door & 0x20) >> 2) | ((door & 0x08) << 1));
		...
		case 0x49: {
			if (CarUtil.getCarType2() > 0) {
				if ((data[2] & 0x01) != 0) {					
					...
					if (null != handler) {
						handler.sendMessage(handler.obtainMessage(
								CANBOX_MAZDA_RAISE_UI_DATA, (data[2] & 0xff)
										| ((data[3] & 0xff) << 8), 0));
					}
				} else {
		...
		case 0x4a:
			if (CarUtil.getCarType2() == 2) {
				if (OSProManager.mReverse != 1
						&& OSProManager.mSimulationReverse != 1) {
					if ((data[2] & 0x80) != 0) {
