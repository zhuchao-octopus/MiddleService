...
//	}
	...
	{ 0x2, KEY_HOME }, { 0x3, KEY_PREVIOUSSONG }, { 0x4, KEY_NEXTSONG },
			{ 0x7, KEY_PLAYPAUSE }, { 0x8, KEY_BACK }, { 0x10, KEY_SOURCE },
			{ 0x11, KEY_SOURCE }, { 0x12, KEY_SEEK_NEXT },
			{ 0x13, KEY_SEEK_PREV }, { 0x14, AK_KEYPAD_VOLUME_A },
			{ 0x15, AK_KEYPAD_VOLUME_D }, { 0x16, KEY_MUTE },
			{ 0x1F, KEY_MIC }, { 0x50, KEY_BT },
			{ 0x20, MyCmd.Keycode.KEY_CAR_INFO }, { 0x21, KEY_MODE },
			{ 0x23, KEY_BT }, { 0x30, MyCmd.Keycode.MULT_SOURCE_AND_BT },

			{ (byte) 0x17, MyCmd.Keycode.ROLL_NEXT }, 
			{ (byte) 0x18, MyCmd.Keycode.ROLL_PREV },
			...
			{ (byte) 0x9e, KEY_MODE },
			{ (byte) 0x9f, MyCmd.Keycode.KEY_RECENT_APPS },
			...
			{ (byte) 0xb0, KEY_BT_DIAL }, { (byte) 0xb1, KEY_BT_HANG },
			{ (byte) 0xb2, KEY_BACK }, { (byte) 0xb3, KEY_GPS },
			{ (byte) 0xb4, MyCmd.Keycode.RDS_TA_SWITCH },
			{ (byte) 0xb5, KEY_FM }, { (byte) 0xb6, KEY_MEDIA },
			{ (byte) 0xb7, KEY_SET },
			{ (byte) 0xb8, MyCmd.Keycode.KEY_RECENT_APPS },
			{ (byte) 0xb9, KEY_PREVIOUSSONG }, { (byte) 0xba, KEY_NEXTSONG },
			{ (byte) 0xbb, KEY_SEEK_PREV }, { (byte) 0xbc, KEY_SEEK_NEXT },
			{ (byte) 0xbd, KEY_PLAYPAUSE },
			{ (byte) 0xbe, MyCmd.Keycode.VOLUME_ROLL_UP },
			{ (byte) 0xbf, MyCmd.Keycode.VOLUME_ROLL_DOWN },
			{ (byte) 0x81, MyCmd.Keycode.VOLUME_ROLL_UP },
...
//		}
	...
	public void parseCanboxData(byte[] data, int len) {
		...
		case 0x36: {
			...
			String s = String.format("%d%s", temp, mContext.getResources()
	...
	public void setMediaMoreInfo(int source, int play, int total, int time,
		...
		if (MyCmd.SOURCE_DVD == source) {
			...
			data = new byte[] { (byte) 0xc0, 0x8, s, mediaType, (byte) play,
					(byte) total, 0, 0, min, sec };
		} else {
			++play;
			data = new byte[] { (byte) 0xc0, 0x8, s, mediaType,
					(byte) (play & 0xff), (byte) ((play & 0xff00) >> 8),
					(byte) (total & 0xff), (byte) ((total & 0xff00) >> 8), min,
					sec };
		}
