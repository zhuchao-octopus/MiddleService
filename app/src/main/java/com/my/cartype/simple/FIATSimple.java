...
public class FIATSimple extends Canbox {
	...
	private final static byte KEYS_WHEEL[][] = { { 0x4, AK_KEYPAD_VOLUME_A },
			{ 0x5, AK_KEYPAD_VOLUME_D }, { 0x9, KEY_NEXTSONG },
	...
	public void parseCanboxData(byte[] data, int len) {
		...
		case 0x14: {
			...
			door = (((door & 0x80) >> 7) | ((door & 0x40) >> 5)
					| ((door & 0x20) >> 3) | ((door & 0x10) >> 1) | ((door & 0x08) << 1));
				...
				if (!"com.canboxsetting/com.canboxsetting.CarInfoActivity"
					...
					try {
						...
						it.setClassName("com.canboxsetting",
								"com.canboxsetting.CarInfoActivity");
	...
	// setMediaSrc(0);
		...
		case MyCmd.SOURCE_RADIO:
			...
			byte[] data = new byte[] { (byte) 0x93, 0x6, 0x1, b[0], 0, 0, b[2],
					b[1] };
			sendDataToCanbox(data, data.length);
			break;
		}
	...
	public void setMediaSrc(int source) {// default is simple box
		...
		if (mMediaType != mediaType) {
			byte[] data = new byte[] { (byte) 0x93, 0x6, mediaType, 0, 0, 0, 0,
					0 };
			sendDataToCanbox(data, data.length);
		}
	}
