...
public class CarGMSimple extends Canbox {
	...
	private final static byte KEYS_WHEEL[][] = { 
		{ 0x1, AK_KEYPAD_VOLUME_A },
			...
			{ (byte)0x81, MyCmd.Keycode.BT_HANG }, 
			{ (byte)0x82, MyCmd.Keycode.BT_DIAL }, 
			{ (byte)0x83, MyCmd.Keycode.BT_DIAL }, 
	...
	private byte[] mAirData = new byte[8];
...
//			sendCanboxInfo("com.canboxsetting", data);
	...
	public void parseEQ(int id, byte[] buf) {
		...
		mEQData[0] = (byte) (buf[6] );
		mEQData[1] = (byte) (buf[7] );
		mEQData[2] = (byte) (buf[8] );
		mEQData[3] = (byte) (buf[5] );
		mEQData[4] = (byte) (buf[4] );
		if ((buf[2] & 0x1) == 0x1) {
			mEQData[5] = 0;
		} else {
			mEQData[5] = buf[3];
