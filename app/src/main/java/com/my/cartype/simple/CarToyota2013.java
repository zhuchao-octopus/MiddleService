...
public class CarToyota2013 extends Canbox {
	...
	private final static byte KEYS_WHEEL[][] = { 
		...
		{ 0x3, MyCmd.Keycode.DVD },
		{ 0x4, MyCmd.Keycode.AUX_IN },
		{ 0x5, MyCmd.Keycode.AUDIO },
		{ 0x6, MyCmd.Keycode.AUDIO },
		{ 0x7, MyCmd.Keycode.BT_MUSIC },
		{ 0x8, MyCmd.Keycode.KEY_TV },
...
//				Log.e(TAG, e.getMessage());
	...
	public void touchInReverse(int x, int y, int w, int h) {
		...
		if (mTNGA360) {
			...
			byte[] buf = new byte[] { (byte) 0x85, 0x5,
					(byte) ((x & 0xff00) >> 8),
					(byte) (x & 0xff),
					(byte) ((y & 0xff00) >> 8),
					(byte) (y & 0xff),0 };
			...
			if (OSProManager.mReverse == 1
					&& OSProManager.mSimulationReverse == 0) {
				if (CarUtil.getKeyType() == 0) {

					if (x > (w - mButtonW) && y > (h - mButtonH)) {
						button = 3;
					} else if ((x < (w - mButtonW))
							&& x > ((w / 2 - mButtonW))
							&& y > (h - mButtonH)) {
						button = 7;
			...
			if (button != 0) {
				...
				if ((button & 0xff00) == 0) {
					...
					buf = new byte[] { (byte) 0x83, 0x2,
							(byte) ((button & 0xff00) >> 8),
							(byte) (button & 0xff) };
				}
				sendDataToCanbox(buf, buf.length);
			}
	...
	public void setMediaMoreInfo(int source, int play, int total, int time,
		...
		//
		// } else {
		data = new byte[] { (byte) 0xc0, 0x8, s, s2, (byte) ((play) & 0xFF),
				(byte) ((play & 0xFF00) >> 8), 0, h, min, sec };
		// }
	...
	public void setPhoneEx(int status, String num, String name) {
		...
		if (mPhoneStatus == status && isStringEqual(num, mPhoneNum)
				&& isStringEqual(name, mPhoneName)) {			
			return;
		}
		mPhoneName = name;
		mPhoneNum = num;
		...
		if (mBTStatusSend != s) {
			mBTStatusSend = s;
			data2 = new byte[] { (byte) 0xc0, 0x8, 0x5, 0x40, (byte) s, 0, 0,
					0, 0, 0 };
