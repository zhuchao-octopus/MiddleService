...
public class HondaDABNR extends Canbox{

	public HondaDABNR(){

		buildCmdRepeatSendCarType(getCarTypeCmd(), 8);
	...
	private void initKey(){
		...
		case 10:
		case 14:
			KEYS_WHEEL[9][1] =  MyCmd.Keycode.MUTE;
			break;
		}
	}
	private byte mKeyPannel[][];
	private static int KEYS_WHEEL[][] = { 
		{ 0x1, AK_KEYPAD_VOLUME_A },
			...
			{ 0x17, KEY_HOME }, 
			{ 0x18, MyCmd.Keycode.KEY_DISPLAY }, 
			{ 0x19, MyCmd.Keycode.ALL_APP }, 
			

			{ 0x20, AK_KEYPAD_VOLUME_A },
			{ 0x21, AK_KEYPAD_VOLUME_D }, 

			{ 0x22, MyCmd.Keycode.KEY_TURN_A },
			{ 0x23, MyCmd.Keycode.KEY_TURN_D }, 

			{ 0x24, MyCmd.Keycode.KEY_SEEK_NEXT },
			{ 0x25, MyCmd.Keycode.KEY_SEEK_PREV }, 			

			{ 0x26, MyCmd.Keycode.NEXT },
...
//			{ 0x29, MyCmd.Keycode.KEY_SIDE_CAMERA }, 
			...
			{ 0x31, KEY_POWER }, 
			{ 0x32, KEY_BACK }, 
			{ 0x33, MyCmd.Keycode.KEY_FM }, 
			{ 0x34, MyCmd.Keycode.KEY_AM  }, 
			{ 0x35, KEY_DVD }, 
			{ 0x36, KEY_AUX }, 
			{ 0x37, KEY_BT }, 
			{ 0x38, MyCmd.Keycode.NUMBER1 }, 
			{ 0x39, MyCmd.Keycode.NUMBER2 }, 
			{ 0x3a, MyCmd.Keycode.NUMBER3 }, 	
			{ 0x3b, MyCmd.Keycode.NUMBER4 }, 	
			{ 0x3c, MyCmd.Keycode.NUMBER5 }, 	
			{ 0x3d, MyCmd.Keycode.NUMBER6 }, 	
			{ 0x3e, MyCmd.Keycode.PLAY_PAUSE }, 	
			{ 0x3f, MyCmd.Keycode.BRIGHTNESS }, 	
			
			{ (byte)0x81, AK_KEYPAD_VOLUME_A },
			{ (byte)0x82, AK_KEYPAD_VOLUME_D }, 
			{(byte) 0x83, KEY_MUTE }, 		
			{(byte) 0x86, KEY_MUTE }, 		
			{(byte) 0x91, MyCmd.Keycode.AUDIO }, 	
			{(byte) 0x92, MyCmd.Keycode.NEXT }, 	
			{(byte) 0x93, MyCmd.Keycode.PREVIOUS }, 	
			{(byte) 0x94, MyCmd.Keycode.KEY_TURN_A }, 	
			{(byte) 0x95, MyCmd.Keycode.KEY_TURN_D }, 	
//			{(byte) 0x96, MyCmd.Keycode }, 	
	
	};

	private final static int KEYS_SOURCE[][] = { 
		{ 0x20, MyCmd.Keycode.KEY_AM },		
		{ 0x21, MyCmd.Keycode.KEY_FM },		
		{ 0x22, MyCmd.Keycode.AUDIO },		
		{ 0x23, MyCmd.Keycode.BT_MUSIC },		
		{ 0x24, MyCmd.Keycode.AUX_IN },		
	...
	private void parseWheelKey(byte[] data) {
		...
		if ((data[2] & 0xff) == 0x96) {
			switch(data[3]){
			case 0x11:
				key = MyCmd.Keycode.KEY_AM;
				break;
			case 0x41:
				key = MyCmd.Keycode.KEY_FM;
				break;
			case 0x23:
				key = MyCmd.Keycode.BT;
				break;
			case 0x22:
				key = MyCmd.Keycode.AUDIO;
				break;
//			case 0x45:
//				key = MyCmd.Keycode.KEY_AM;
//				break;
...
//		}
		...
		airData[0] = (byte) ((data[2] & 0xec) | ((data[6] & 0x80) >> 6) |
				((data[2] & 0x02) >> 1));
		...
		if (airData[0] != (byte) (mAirData[0] & 0xff)
				...
				|| airData[8] != (byte) (mAirData[8] & 0xff)
				|| airData[10] != (byte) (mAirData[10] & 0xff)
				|| airData[11] != (byte) (mAirData[11] & 0xff)) {
			airControl = true;
...
//		}
	...
	public void updateTime() {
		...
		byte []buf = new byte[] { (byte) 0xc6, 0x04, 0x50, h,
				m, s};
		sendDataToCanbox(buf, buf.length);
	}
	...
	private void toggleCameraSwitch() {
		...
		Intent it = new Intent(Intent.ACTION_VIEW);
		if ((top != null
				&& top.contains("com.my.frontcamera.SideCameraActivity"))) {
...
//				Log.e(TAG, e.getMessage());
	...
	public void parseEQ(int id, byte[] buf) {
		...
		mEQData[0] = (byte) (buf[5] );
		mEQData[1] = (byte) (buf[6] );
		mEQData[2] = (byte) (buf[7] );
		mEQData[3] = (byte) (buf[3] );
