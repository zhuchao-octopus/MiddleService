...
//		buildCmdEQ((byte) 0x27, (byte) 0x0, 6);
	...
	public void parseACInfo(byte[] data)
	{
		...
		airData[1] = (byte) (((data[6] & 0x0f))				
				| ((data[6] & 0x40)>>1) 			
				| ((data[6] & 0x20)<<1) 			
				| ((data[6] & 0x10)<<3) );
		
		airData[6] = (byte) (((data[7] & 0x0f))				
				| ((data[7] & 0x40)>>1) 			
				| ((data[7] & 0x20)<<1) 			
				| ((data[7] & 0x10)<<3) );
...
//		mHandler.sendEmptyMessageDelayed(0, 4000);
	...
	public void setMediaMoreInfo(int source, int play, int total, int time,
		...
		String s = String.format("%02d:%02d:%02d", (time) / 3600, (time) / 60,
				(time) % 60);
	...
	public void setMediaSrc(int source, byte type, byte[] b) {
		...
		case MyCmd.SOURCE_RADIO: {
			...
			if (b[0] >= 0x10) { // am
				...
				s = ""
						+ String.format("%d.%02d", (freq) / 100, (freq) % 100, Locale.ENGLISH);
				if (freq < 10000){
					s = " " + s;
				}
				copyLcdInfo(mLcdInfo, s);
