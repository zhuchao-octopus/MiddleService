...
public class RecentView {
	...
	public RecentView(Context context) {
		...
		mTrList = (ListView) mViewMicButton.findViewById(R.id.list_view);
...
//				openActivity(position);
	...
	private class MyListAdapter extends BaseAdapter {
		...
		private View newView(ViewGroup parent) {
			...
			vh.name = (TextView) v.findViewById(R.id.text);
			vh.bg =  v.findViewById(R.id.img);
			vh.bt = (ImageView) v.findViewById(R.id.close);
			v.setTag(vh);
			return v;
		}
		...
		for (index = 0; index < numTasks; ++index) {
			if (index < numTasks) {
				...
				if (info != null) {
					...
					Log.d("ffck1", info.toString() + "1loadThumbnail:" + b);
