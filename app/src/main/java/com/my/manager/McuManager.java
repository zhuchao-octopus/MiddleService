...
public class McuManager {
	...
	public static McuManager getInstanse(Context c) {
	...
	public static McuManager getInstanse() {
		return mThis;
	}
	...
	private void doMcuData(byte[] param) {
		...
		Log.e(TAG, "doMcuData:" + Util.byteArrayToHex(param));
		if (mRealPowerOff) {
			Log.d(TAG, "mRealPowerOff:" + mRealPowerOff);
			return;
		}
		try {
			switch (param[0]) {
			...
			case ProtocolAk47.TYPE_COMMON_RECEIVE:
				...
				case 4:
					if (Util.isRKSystem()) {
						boolean on = false;
						if (param[3] == 4 || param[3] == 1) {
							on = true;
						}
						CarUtil.setCanboxLED(CarUtil.CANBOX_LED_ILL, on ? 0
								: 1);
						if (GlobalDef.mIsTesting) {
							if (on) {
				...
				case 8:
					if (param[2] == 0x20){//battery
						...
						Intent it = new Intent(
								MyCmd.BROADCAST_CAR_SERVICE_SEND);
						it.putExtra(MyCmd.EXTRA_COMMON_CMD,
								MyCmd.Cmd.MCU_RETURN_VOLTAGE_PROTECT );
						it.putExtra(MyCmd.EXTRA_COMMON_DATA, param[3]);
						mContext.sendBroadcast(it);
				...
				case 0xd:
					...
					break;
				case 0xe:
					updateSaveTime(((param[2] & 0xff) << 24)
							| ((param[3] & 0xff) << 16)
							| ((param[4] & 0xff) << 8)
							| ((param[5] & 0xff) << 0));
					break;
			...
			case ProtocolAk47.TYPE_RADIO_RECEIVE:
				if (param[1] == 0x2) {
					if (GlobalDef.mIsTesting) {
						Log.d("test_radio", GlobalDef.mIsTesting + "doMcuData:"
								+ Util.byteArrayToHex(param));
						byte[] cmd = new byte[] { AutoTest.RADIO_RETURN,
								param[6] };
...
//						MyCmd.Cmd.MCU_RADIO_RECEIVE_DATA, param);
			...
			case ProtocolAk47.TYPE_AUDIO_RECEIVE:
				...
				case ProtocolAk47.RECEVE_AUDIO_VOLUME_INFO:
					if (mVolume != -1) {
						if (!mPowerOffFate || (mBtPhoneStatus == MyCmd.PhoneStatus.PHONE_ON)) {
							if (!mHideVolumeUI) {
								BroadcastUtil
										.sendByCarService(
												...
												MyCmd.Cmd.MCU_AUDIO_RECEIVE_DATA,
												param);
							} else {
							...
							if (mHideVolumeUI) {
								Intent itVolume = new Intent(
										MyCmd.BROADCAST_CAR_SERVICE_SEND);
								itVolume.putExtra(MyCmd.EXTRA_COMMON_CMD,
										MyCmd.Cmd.RETURN_CURRENT_VOLUME);
								itVolume.putExtra(MyCmd.EXTRA_COMMON_DATA,
										(int) (param[2] & 0xff));
				...
				case 0xc:
					BroadcastUtil.sendByCarService(mContext,
							AppConfig.PACKAGE_EQ,
							MyCmd.Cmd.MCU_AUDIO_RECEIVE_DATA, param);
					mLoud = param[2];
					break;
				case 0xe:
				case 0xf:
				...
				case 0xd: // spectrum
					BroadcastUtil.sendByCarService(mContext,
							...
							MyCmd.Cmd.RETURN_AUDIO_SPECTRUM, param);
					break;
				case ProtocolAk47.RECEVE_AUDIO_EQ_INFO:
					...
					if (CarUtil.mIsNeedSendEQ) {
						...
						byte[] data = (new byte[] { param[7], param[8],
						...
						mMcuHandler.sendMessageDelayed(mMcuHandler
								.obtainMessage(MSG_SEND_EQ_TO_CANBOX, data),
								300);
			...
			case ProtocolAk47.TYPE_DVD_RECEIVE:
				...
				BroadcastUtil.sendByCarService(mContext,
						AppConfig.PACKAGE_CAR_UI,
						MyCmd.Cmd.MCU_DVD_RECEIVE_DATA, param);
				break;
			case ProtocolAk47.TYPE_SETTINGS_RECEIVE:
				...
				case 0x11:
					...
					BroadcastUtil.sendByCarService(mContext,
							"com.my.filemanager", MyCmd.Cmd.SET_VCOM, vcom);
					break;
				}
				break;
			case ProtocolAk47.TYPE_SWC_RECEIVE:
				// case ProtocolAk47.TYPE_PANEL_KEY_SEND:
				if (param[1] == 2 && GlobalDef.mIsTesting) {
			...
			Log.d(TAG, "doMcuData exception:" + Util.byte2HexStr(param));
		}
	}
				...
				if (mAppSource == MyCmd.SOURCE_NONE) {
					...
					else 
					{
						if (msg.arg1 < 40) {
							...
							mMcuHandler.sendMessageDelayed(mMcuHandler
									.obtainMessage(MSG_FIRST_RUN_POWERON,
											msg.arg1 + 1, 0), 500);
						}
					}
				}
	...
	public int setMcuBTType(int i) {
		return mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_SETTINGS_SEND, (byte) 0x11, (byte) i));
	}

	public int setMcuBacklight(int i) {
		return mMcu.sendCmd(ProtocolAk47.generateProtocol2(
				ProtocolAk47.TYPE_COMMON_SEND,
				ProtocolAk47.SEND_COMMON_SUB_BRIGHTNESS, (byte) i, (byte) i));//for kld px5&& 3566 pg
	}

	public int setMcuSource(int i) {
		return mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_COMMON_SEND,
				ProtocolAk47.SEND_COMMON_SUB_FRONT_SOURCE, (byte) i));
	}

	public int setMcuSceen0(int i) {
		return mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_COMMON_SEND,
				ProtocolAk47.SEND_COMMON_SUB_SCREEN0, (byte) i));
	}

	public void setVoulume(int i) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol2(
				ProtocolAk47.TYPE_AUDIO_SEND,
				ProtocolAk47.SEND_VOLUME_SUB_VOLUME_CONTROL,
				ProtocolAk47.SEND_VOLUME_SUB_DATA1_SET_VOLUME, (byte) i));
	}

	public void setEQZoneUsed(int i) {
		mMcu.sendCmd(ProtocolAk47
				.generateProtocol2(ProtocolAk47.TYPE_AUDIO_SEND, (byte) 0x2,
						(byte) 0x50, (byte) i));
	}
	
	public void setVoulumeMute(int i) {
		mMcu.sendCmd(ProtocolAk47
				.generateProtocol2(ProtocolAk47.TYPE_AUDIO_SEND,
						ProtocolAk47.SEND_VOLUME_SUB_VOLUME_CONTROL, (byte) 1,
						(byte) i));
	}
	...
	public void setVoulumeAutoMuteTime(int time) { // time is 100ms
		mMcu.sendCmd(ProtocolAk47.generateProtocol2(
				ProtocolAk47.TYPE_AUDIO_SEND,
				ProtocolAk47.SEND_VOLUME_SUB_VOLUME_CONTROL, (byte) 0x4,
				(byte) time));
	}
	...
	public void setMcuBtPhoneStatus(int i) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_COMMON_SEND,
				ProtocolAk47.SEND_COMMON_SUB_BLUETOOTH, (byte) i));
	}

	// radio
	public void setRadio(int subId, int param1, int param2) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol2(
				ProtocolAk47.TYPE_RADIO_SEND, (byte) subId, (byte) param1,
				(byte) param2));
	}
	...
	public void setRadioRds(int subId, int param1, int param2) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol2(ProtocolAk47.TYPE_RDS_SEND,
				(byte) subId, (byte) param1, (byte) param2));
	}

	public void setRadioRds(int subId, int param1) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(ProtocolAk47.TYPE_RDS_SEND,
				(byte) subId, (byte) param1));
	}
	...
	public void setAudio(int subId, int param1) {
		if (subId != 0xd) {
			mMcu.sendCmd(ProtocolAk47.generateProtocol1(
					ProtocolAk47.TYPE_AUDIO_SEND, (byte) subId, (byte) param1));
		} else {
			byte[] protocol = new byte[] { ProtocolAk47.TYPE_AUDIO_SEND,
					(byte) subId, (byte) ((param1 & 0xff) << 0),
					(byte) ((param1 & 0xff00) >> 8),
					(byte) ((param1 & 0xff0000) >> 16),
					(byte) ((param1 & 0xff000000) >> 24) };
	...
	public void setAudio(int subId, int param1, int param2, int param3) {
		byte[] protocol = new byte[] { ProtocolAk47.TYPE_AUDIO_SEND,
				(byte) subId, (byte) param1, (byte) param2, (byte) param3 };
		mMcu.sendCmd(protocol);
	}
	...
	public void setSpectrumSwitch(byte i) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_AUDIO_SEND, (byte) 0xf, (byte) i));
	}

	public void queryVoulume() {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_AUDIO_SEND, (byte) 0x6, (byte) 0x4));
	}

	// dvd
	public int setDvd(int subId, int param1, int param2) {
		return mMcu.sendCmd(ProtocolAk47.generateProtocol1((byte) subId,
				(byte) param1, (byte) param2));
	}

	public void setDvd(int subId, int param1) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(ProtocolAk47.TYPE_DVD_SEND,
				(byte) subId, (byte) param1));
	}
	...
	public void setTV(int param) {
		...
		if (subId == 0x3) {
			mMcu.sendCmd(ProtocolAk47.generateProtocol2(
					ProtocolAk47.TYPE_TV_SEND, (byte) subId,
					(byte) ((param & 0xff00) >> 8), (byte) (param & 0xff)));
		} else {
			mMcu.sendCmd(ProtocolAk47.generateProtocol1(
					ProtocolAk47.TYPE_TV_SEND, (byte) subId,
					(byte) (param & 0xff)));
	...
	public void setVCOM(int subId, int param1) {
		if (subId == 0) {
			mMcu.sendCmd(ProtocolAk47.generateProtocol2(
					ProtocolAk47.TYPE_SETTINGS_SEND, (byte) 0x10, (byte) subId,
					(byte) param1));
		} else if (subId == 0x10) {
			mMcu.sendCmd(ProtocolAk47.generateProtocol2(
					ProtocolAk47.TYPE_SETTINGS_SEND, (byte) 0x10, (byte) subId,
	...
	public void setRadioAntPower(int power) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_RADIO_SEND, (byte) 0x5, (byte) power));
	...
	public void setSwcKeyStudy(int param1) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(ProtocolAk47.TYPE_SWC_SEND,
				(byte) 0x1, (byte) param1));
	}

	public void setSystemReadyWatchDog() {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_COMMON_SEND,
				ProtocolAk47.SEND_COMMON_SUB_POWER, (byte) 0xff));
	}

	public void clearSystemReadyWatchDog() {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_COMMON_SEND,
				ProtocolAk47.SEND_COMMON_SUB_POWER, (byte) 0xfe));
	}
...
//		Log.d("ffck", "setFrontCamerPower:" + data + ":" + user);
		if (data == 0) {
			...
			if (mFCPowerUser == 0) {
				mMcu.sendCmd(ProtocolAk47
						.generateProtocol1(ProtocolAk47.TYPE_COMMON_SEND,
								(byte) 0x1c, (byte) data));
			}
		} else {
			mFCPowerUser |= user;
			mMcu.sendCmd(ProtocolAk47.generateProtocol1(
					ProtocolAk47.TYPE_COMMON_SEND, (byte) 0x1c, (byte) data));
		}
	}
	// do key
	...
	private int doKeyType(int key) {
		if (GlobalDef.mPannelKeyType == 1) {
			if (key == MyCmd.Keycode.UP || key == MyCmd.Keycode.DOWN
					|| key == MyCmd.Keycode.LEFT || key == MyCmd.Keycode.RIGHT
					|| key == MyCmd.Keycode.ENTER) {
	...
	public int doKey(int key) {
		...
		if (key >= MyCmd.Keycode.CANBOX_FUNCTION_START &&
				key <= MyCmd.Keycode.CANBOX_FUNCTION_END){
			Canbox canbox = CarUtil.getCanboxInstance();
			if (canbox!=null){
	...
	//	Log.d(TAG, ">>doKey:" + key);
		if ((mLockKey == LOCK_KEY_ALL)
				|| (mLockKey == LOCK_KEY_EXCEPTION_POWER && key != MyCmd.Keycode.POWER && key != (MyCmd.Keycode.POWER_ON&0xff))) {
			...
			if (MyCmd.Keycode.IXB_360_DISPLAY != key) {
				if (mBtPhoneStatus == MyCmd.PhoneStatus.PHONE_ON
						&& (key == MyCmd.Keycode.BT
								|| key == MyCmd.Keycode.BT_DIAL || key == MyCmd.Keycode.BT_HANG
								|| key == MyCmd.Keycode.VOLUME_DOWN || key == MyCmd.Keycode.VOLUME_UP)) {
	...
	//	Log.d(TAG, "doKey: lock by" + key);
		...
		GlobalDef.wakeLockOnce();
		if (GlobalDef.mIsTesting) {
			Log.d(TAG, "doKey: GlobalDef.mIsTesting" + GlobalDef.mIsTesting);
			return 0;
		}
		...
		case MyCmd.Keycode.EASY_CONNECT:
			UtilSystem.doRunActivity(mContext, "net.easyconn",
				"net.easyconn.ui.Sv04MainActivity");
			break;
		case MyCmd.Keycode.KEY_F_CAMERA:
			UtilSystem.doRunActivity(mContext, "com.car.ui",
				"com.my.frontcamera.FrontCameraActivity");
			break;
		case MyCmd.Keycode.KEY_CAMERA:
			UtilSystem.doRunActivity(mContext, "com.car.ui",
				"com.my.frontcamera.BackCameraActivity");
			break;
		case MyCmd.Keycode.KEY_SIDE_CAMERA:
			UtilSystem.doRunActivity(mContext, "com.car.ui",
				"com.my.frontcamera.SideCameraActivity");
			break;
		case MyCmd.Keycode.KEY_CAR_INFO:
			try {
			...
			break;
		case MyCmd.Keycode.KEY_CAR_SETTING:
			UtilSystem.doRunActivity(mContext, "com.canboxsetting",
		...
		case MyCmd.Keycode.KEY_RECENT_APPS:
			GlobalDef.sendByCarServiceToSystemUI(mContext,
					"com.android.systemui", MyCmd.Cmd.SHOW_RECENT_APPS);
			break;
		case MyCmd.Keycode.KEY_SCREENT_SHOT :
			GlobalDef.sendByCarServiceToSystemUI(mContext,
					"com.android.systemui", MyCmd.Cmd.DO_SCREEN_SHOT);
			break;
		case MyCmd.Keycode.KEY_SPLIT_WINDOW :
			GlobalDef.sendByCarServiceToSystemUI(mContext,
					"com.android.systemui", MyCmd.Cmd.SYSTEMUI_LONG_PRESS_RECENT);
			break;
		case MyCmd.Keycode.KEY_SPEED :
			GlobalDef.sendByCarServiceToSystemUI(mContext,
					"com.android.systemui", MyCmd.Cmd.SYSTEMUI_SPEED_UP);
			break;
...
//			try {
//				Intent it = new Intent(Intent.ACTION_VIEW);
//				it.setClassName("com.car.ui", "com.my.frontcamera.FrontCameraActivity");
//				it.setFlags(Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT
//						| Intent.FLAG_ACTIVITY_NEW_TASK);
//				mContext.startActivity(it);
//			} catch (Exception e) {
...
//			break;
			...
			mHandleModeKey.postDelayed(new Runnable() {
				...
				public void run() {
					BroadcastUtil.sendByCarService(mContext,
							...
							MyCmd.Cmd.LAUNCHER_SHOW_ALL_APP);
				}
			}, delay);
		}
			break;
	...
	private void broadcasKeyToApplication(int key) {
		...
		case MyCmd.Keycode.NUMBER9:
			if (mBtPhoneStatus == MyCmd.PhoneStatus.PHONE_ON) {
				BroadcastUtil.sendKey(mContext, "com.my.bt", key);
				send = true;
			}
			break;
	...
	private boolean isOwerAppControlSource() {
		if (mAppSource != MyCmd.SOURCE_RADIO && mAppSource != MyCmd.SOURCE_DVD
				&& mAppSource != MyCmd.SOURCE_AUX
				&& mAppSource != MyCmd.SOURCE_DTV
				&& mAppSource != MyCmd.SOURCE_DTV_CVBS
				&& mAppSource != MyCmd.SOURCE_BT_MUSIC
				&& mAppSource != MyCmd.SOURCE_MUSIC
				&& mAppSource != MyCmd.SOURCE_VIDEO
				&& mAppSource != MyCmd.SOURCE_DVR
				&& mAppSource != MyCmd.SOURCE_DAB
				&& mAppSource != MyCmd.SOURCE_USBDVD) {
			return false;
		}
		return true;
	}

	private final static String[] USE_JOY_APPLICATION = { "com.car.ui",
			"com.my.bt", 
	...
	private void prepareKeyEject() {
		if (mDVDStatus == DVD_STATUS_DISK_IN_ENTRY) {
			...
			if (mDVDPower != 0) {
				Util.setFileValue("/sys/class/misc/ak-dvd/device/data",
						"0x0212");
				Util.doSleep(5);
				Util.setFileValue("/sys/class/misc/ak-dvd/device/data",
						"0x0212");
				...
				Util.doSleep(5);
				Util.setFileValue("/sys/class/misc/ak-dvd/device/data",
						"0x0300"); // save play status

			}
			if (!mDvdEjecting) {
				mDvdEjecting = true;
				mMcuHandler.sendEmptyMessageDelayed(MSG_DVD_EJECT,
						TIME_DVD_EJECT);
			}
		}
	}
	...
	private Toast makeModeToast() {
		...
		mTextViewMode = (TextView) v.findViewById(R.id.mode_text);
	...
	private List<mModeKeyConfig> mListModeKey = new ArrayList<mModeKeyConfig>();
	...
	private boolean initModeKeyContent(){
		if (mToastModeKey != null){			 
			...
			if (s != null){
				...
				if (s.contains("video")) {
					mListModeKey.add(new mModeKeyConfig(MyCmd.Keycode.VIDEO,
							R.string.button_text_video));
				}
				if (s.contains("bt_music")) {
					mListModeKey.add(new mModeKeyConfig(MyCmd.Keycode.BT_MUSIC,
							R.string.button_text_bt_music));
				}
				if (s.contains("aux")) {
					mListModeKey.add(new mModeKeyConfig(MyCmd.Keycode.AUX_IN,
	...
	private Handler mHandleModeKey = new Handler() {
		@Override
		public void handleMessage(Message msg) {
			Log.d(TAG, "mHandleModeKey:" + msg.what);
			switch (msg.what) {
			case 0:
	...
	private void doAccPower(int param) {
		...
		case 2:
			if (Util.isRKSystem()) {
				if ((SystemClock.uptimeMillis() - GlobalDef.mSystemBootStartTime) > 5000) {
					mTagAccPower = 1;
					doAccPowerOffFate();
	...
	public void sendAccState(Context context, int state) { //suding use
		...
		context.getContentResolver().notifyChange(
				Settings.System.getUriFor(ACC_STATE), null);
	}
	...
	private void doAccPowerOffFate() {
		Log.d(TAG, "doAccPowerOffFate:" + mPowerOffAcc + "run time:"
				+ (SystemClock.uptimeMillis() - mSystemStartTime));
		if (!mPowerOffAcc) {
			mPowerOffAcc = true;
	...
	private void doAccPowerOffFatePx5() {
		saveData(SAVE_DATA_TIME,
				(System.currentTimeMillis() + TIME_DELAY_REAL_POWEROFF));
		storeEQIndepend();
		...
		if (Util.isRKSystem()) {
			...
			if (mSystemStartTime != 0
					&& ((SystemClock.uptimeMillis() - mSystemStartTime) > SLEEP_LONG_TIME_TO_REBOOT)) {
				Log.d(TAG, "doAccPowerOffFate: run too much");
				mMcu.sendCmd(ProtocolAk47.generateProtocol1(
						ProtocolAk47.TYPE_COMMON_SEND, (byte) 0x16, (byte) 0x2));
			}
			mHandleModeKey.removeMessages(MSG_ACC_POWEROFF);
			mHandleModeKey.sendEmptyMessageDelayed(MSG_ACC_POWEROFF,
					TIME_DELAY_REAL_POWEROFF);
		}
	}
...
//					0);
	...
	private void sendPowerKeySwitchMcu(int b){//0x80 query
		...
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_COMMON_SEND, (byte) 0x1d, (byte)b));
	}
...
//			doBacklight(true);
			...
			if (ui) {
				if (mPowerOffDialog == null) {
					...
					mPowerOffDialog.setTitle(mContext
							.getText(com.android.internal.R.string.power_off));
					mPowerOffDialog
							.setMessage(mContext
									.getText(com.android.internal.R.string.shutdown_progress));
					mPowerOffDialog.setIndeterminate(true);
					mPowerOffDialog.setCancelable(false);
					mPowerOffDialog.getWindow().setType(
			...
			if (updateTinyRam) {
				...
				Log.d("Mcu", mPowerKeySwitch + "tinyram set mAppSource:"
						+ mAppSource);
			...
			mHandleModeKey.sendEmptyMessageDelayed(MSG_AFTER_FATE_POWEROFF,
					2000);
			mContext.sendBroadcast(new Intent(MyCmd.BROADCAST_POWER_OFF));
			mMcu.sendCmd(ProtocolAk47.generateProtocol1(
					ProtocolAk47.TYPE_COMMON_SEND, (byte) 0x16, (byte) 0x1));
			...
			mMcu.sendCmd(ProtocolAk47.generateProtocol1(
					ProtocolAk47.TYPE_COMMON_SEND, (byte) 0x16, (byte) 0x0));
			doPowerOn();
	...
	public void queryBattery() {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1(
				ProtocolAk47.TYPE_COMMON_SEND,
				ProtocolAk47.SEND_COMMON_SUB_POWER, (byte) 0x20));
	}
...
//		}, 4000);
	...
	public void doPowerOn() {
		...
		case MyCmd.SOURCE_RADIO:
			name = "com.my.radio.RadioActivity";
			break;
		case MyCmd.SOURCE_MUSIC:
			name = "com.my.audio.MusicActivity";
			break;
		...
		case MyCmd.SOURCE_VIDEO:
			name = "com.my.video.VideoActivity";
			break;
		case MyCmd.SOURCE_BT_MUSIC:
			if (!mBtMusicInBTapk) {
				name = "com.my.btmusic.BTMusicActivity";
			} else {
				pacageName = "com.my.bt";
				name = "com.my.bt.ATBluetoothActivity";
			}
			break;
		case MyCmd.SOURCE_AUX:
			name = "com.my.auxplayer.AUXPlayer";
			break;
		case MyCmd.SOURCE_DVD:
			// setDvd(2, 0);
			name = "com.my.dvd.DVDPlayer";
			break;
		case MyCmd.SOURCE_DTV:
		case MyCmd.SOURCE_DTV_CVBS:
			name = "com.my.tv.TVActivity";
			break;
		...
		if (name != null) {
			...
			if (!showScreen1) {
				try {
					...
					if (!mBtMusicInBTapk
							|| !name.equals("com.my.bt.ATBluetoothActivity")) {
						mContext.startActivity(it);
					} else {
						mMcuHandler.postDelayed(new Runnable() {
							public void run() {
	...
	private void doKeyLoudness() {
		...
		mToastLoud = Toast.makeText(mContext, "Loud "
				+ mContext.getResources().getString(id), Toast.LENGTH_SHORT);
		mToastLoud.show();
	}
	...
	private void doBacklight(boolean Backlight) {
		...
		GlobalDef.sendByCarServiceToSystemUI(mContext, "com.android.systemui",
				Backlight ? MyCmd.Cmd.BACKLIGHT_ON : MyCmd.Cmd.BACKLIGHT_OFF);
		// if(Backlight){
		//
		// lockTouch(0);
		// }else{
	...
	private byte[] mTinyRam = new byte[4];
	...
	public int setSource(int index) {
		...
		} else if (mAppSource == MyCmd.SOURCE_DVD
				&& index != MyCmd.SOURCE_OTHERS_APPS
				&& index != MyCmd.SOURCE_CANBOX_PHONE) {
			// prepareDvdPowerOff();
			// if (mRearL != MyCmd.SOURCE_DVD && mRearR != MyCmd.SOURCE_DVD) {
			prepareDvdPowerOff();
			// }
		...
		if (index == MyCmd.SOURCE_VIDEO || index == MyCmd.SOURCE_MUSIC
				|| index == MyCmd.SOURCE_USBDVD
				|| index == MyCmd.SOURCE_VIDEO_FILEMANAGE
				|| index == MyCmd.SOURCE_MUSIC_FILEMANAGE
				|| index == MyCmd.SOURCE_OTHERS_APPS
				|| index == MyCmd.SOURCE_DVR) {
	...
	private final static int[] mEQIndex = { MyCmd.SOURCE_NONE,
			MyCmd.SOURCE_RADIO, MyCmd.SOURCE_DVD, MyCmd.SOURCE_AUX,
			MyCmd.SOURCE_MX51, MyCmd.SOURCE_BT };

	private int[] mEQValue = new int[mEQIndex.length];
	...
	private class SettingsObserver extends ContentObserver {
		...
		void startObserving() {
			...
			resolver.registerContentObserver(Settings.System
					.getUriFor(SystemConfig.KEY_EQ_INDEPEND_SWITCH), false,
					this);
		}
	...
	private void initEQIndepend() {
		...
		try {
			mEQindpenOn = Settings.Global.getInt(mContext.getContentResolver(),
					SystemConfig.KEY_EQ_INDEPEND_SWITCH);
	...
	private void doDvdStatusChange(byte s) {
		if (s == DVD_STATUS_DISK_INSIDE ||
				s == DVD_STATUS_DISK_IN_ENTRY ||
				s == DVD_STATUS_IN_ING ||
				s == DVD_STATUS_OUT_ING ){
			GlobalDef.makeSureDVDExist(true);
		}
		if (s == DVD_STATUS_DISK_INSIDE ||
				s == DVD_STATUS_DISK_IN_ENTRY ||
				s == DVD_STATUS_IN_ING){				
		...
		if (mDVDStatus != s) {
			...
			if (/* s == DVD_STATUS_IN_ING || */s == DVD_STATUS_DISK_INSIDE
					&& (AppConfig.CAR_UI_DVD.equals(AppConfig.getTopActivity()) || mDVDStatus == DVD_STATUS_IN_ING)) {
				dvdPowerOn();
			} else {
				Log.d(TAG, "doDvdStatusChange:" + s + ":" + mDVDStatus + ":"
						+ AppConfig.getTopActivity());
			}
	...
	private void prepareDvdPowerOff() {
		...
		mMcuHandler.removeMessages(MSG_DVD_POWER_OFF);
		mMcuHandler.sendEmptyMessageDelayed(MSG_DVD_POWER_OFF,
				TIME_DVD_POWER_OFF);
		// }
	...
	public void setCameraSource(int source) {
		if (ReverseManager.isShow && source != MyCmd.SOURCE_REVERSE) {
			...
			Log.d(TAG,
					"setCameraSource: is in reverse, can't set other camera source");
			return;
		}
		switch (source) {
		case MyCmd.SOURCE_AUX:
	...
	public void setScreen0(int index) { // for hdmi tv
		...
		if (index == 0) {
			mMcuHandler.sendMessageDelayed(
					mMcuHandler.obtainMessage(MSG_DELAY_SET_ARM_LVDS, 0, 0),
					200);
			mMcuHandler
					.sendMessageDelayed(mMcuHandler.obtainMessage(
							MSG_DELAY_SET_MCU_LVDS, index, 0), 400);
			mMcuHandler
					.sendMessageDelayed(mMcuHandler.obtainMessage(
							MSG_DELAY_SET_TV_LVDS, 0, 0), 600);
			...
			mMcuHandler
					.sendMessageDelayed(mMcuHandler.obtainMessage(
							MSG_DELAY_SET_TV_LVDS, 1, 0), 600);
			mMcuHandler
					.sendMessageDelayed(mMcuHandler.obtainMessage(
							MSG_DELAY_SET_MCU_LVDS, index, 0), 400);
			mMcuHandler.sendMessageDelayed(
					mMcuHandler.obtainMessage(MSG_DELAY_SET_ARM_LVDS, 4, 0),
					200);
	...
	public void recoverBacklightStatus(int index) {
		...
		if (mBacklightRecover) {
			...
			if (mRecoverDevice[0] == 0 && mRecoverDevice[1] == 0
					&& mRecoverDevice[2] == 0) {
				mBacklightRecover = false;
				doBacklight(false);
			}
	...
	public void setBtPhoneStatus(int i) {
		if (i == MyCmd.PhoneStatus.PHONE_ON) {
			...
			if (mAppSource == MyCmd.SOURCE_BT_MUSIC) {
				mMcu.sendCmd(ProtocolAk47.generateProtocol2(
						ProtocolAk47.TYPE_AUDIO_SEND,
						ProtocolAk47.SEND_VOLUME_SUB_VOLUME_CONTROL, (byte) 4,
						(byte) 10));
			}
			return;
		} else if (MyCmd.PhoneStatus.PHONE_GOC_MUTE == i) {
			if (mAppSource == MyCmd.SOURCE_BT_MUSIC
					|| mBtPhoneStatus == MyCmd.PhoneStatus.PHONE_ON) {
				mMcu.sendCmd(ProtocolAk47.generateProtocol2(
						ProtocolAk47.TYPE_AUDIO_SEND,
						ProtocolAk47.SEND_VOLUME_SUB_VOLUME_CONTROL, (byte) 4,
						(byte) 10));
	...
	public void setCanboxPhoneStatus(int i) {
		if (i == MyCmd.PhoneStatus.PHONE_ON
				|| i == MyCmd.PhoneStatus.PHONE_CARPLAY_ON) {
			...
			requestAudioFocus();
				...
				|| i == MyCmd.PhoneStatus.PHONE_CARPLAY_OFF) {
	...
	private static boolean checkWhiteList(String app) {
		if (app != null) {
			for (int i = 0; i < KILL_WHITELIST.length && KILL_WHITELIST[i] != null; i++) {
				if (KILL_WHITELIST[i].equals(app))
					return true;
			}
		}
		return false;
	...
	private final static String[] KILL_SYSTEM_APP = { "com.android.chrome",
			"com.android.email", "com.google.android.gm",
			"com.google.android.apps.maps", "com.android.vending", "com.carletter.car" };
	...
	private void killAllNoSystemProcess() {
		try {
			...
			List<ActivityManager.RunningAppProcessInfo> mRunningProcess = am
			...
			for (ActivityManager.RunningAppProcessInfo amProcess : mRunningProcess) {
				...
				for (ResolveInfo appInfo : apps) {
					...
					if (amProcess.processName
							.startsWith(appInfo.activityInfo.packageName)) {
						if (flag) {
							kill = 1;
						} else {
							kill = 0;
				...
				Log.d(TAG, amProcess.processName + "<<kill:" + kill + ":"
						+ amProcess.importance);
	...
	private void resetDefaultVolume() {
		if (MachineConfig.VALUE_SYSTEM_UI_KLD7_1992.equals(GlobalDef.mSystemUI)) {
			return;
		}

		int i = SystemConfig.getIntProperty2(mContext,
				SystemConfig.KEY_DEFAULT_RESET_VOLUME_LEVEL);
		if (i != -1) {
			DEFAULT_VOLUME = i;
		}
		Log.d(TAG, "resetDefaultVolume:" + mVolume + "to:" + DEFAULT_VOLUME);
		if (DEFAULT_VOLUME > 0 && mVolume > DEFAULT_VOLUME) {
			setVoulume(DEFAULT_VOLUME);
		}
	}

	private ArrayList<String> mUSBUmount = new ArrayList<String>();
	...
	private void remountUSB(String id) {
		...
		intent.setClassName("com.android.settings",
				"com.android.settings.deviceinfo.StorageMountReceiver");
				...
				for (Object volumeinfo : VolumeInfo) {
					...
					if (diskinfo != null) {
						...
						if (bIsSd && Util.isPX6()) {
							...
							if (strPath != null	&& strPath.startsWith("/storage/MediaCard")
								&& id != null && id.startsWith("public:8,")) {
								Log.w("abcd", "MediaCard " + id + " is actually a UDISK");
								bIsSd = false;
							}
						}
						if (!bIsSd) {
							Log.d("abcd", (path == null ? "path is null," : path.toString())
									+ " ejectAllUsbStorage " + id);
							...
							intent.putExtra(
									"android.os.storage.extra.VOLUME_ID", id);
							intent.addFlags(Intent.FLAG_RECEIVER_FOREGROUND);
							//
							//
							mContext.sendBroadcast(intent);
	...
	private void doKeyCustomApp() {
		...
		String pc = SystemConfig.getProperty(mContext,
				SystemConfig.KEY_CUSTOM_APP);
		if (pc != null) {
			String[] ss = pc.split("/");
			Intent it = new Intent();
			try {
	...
	public void setBrakeProp() {
		if (mBrakeSwitch != 0) {
			if (mBrake != -1) {
				if (mBrake == 0 && GlobalDef.mPreGPSBrake != 1
						&& CarUtil.mPreCanboxBrake != 1) {
					if (!GlobalDef.mTopIsNoNeedBrakeControl){
						Util.setProperty("ak.codec.disable_video_out", "1");
					}
					MyService.doSaveDriver();
				} else {
	...
	private void updateAutoVideoOut(int index) {
		if (AppConfig.isHidePackage("com.my.videoout.VideoOutActivity")) {
			...
			if (index == MyCmd.SOURCE_DVD || index == MyCmd.SOURCE_AUX) {
				...
				mMcu.sendCmd(ProtocolAk47.generateProtocol1(
						ProtocolAk47.TYPE_COMMON_SEND, (byte) 0x3, (byte) 0x7));
				mMcu.sendCmd(ProtocolAk47.generateProtocol1(
						ProtocolAk47.TYPE_COMMON_SEND, (byte) 0xF, (byte) 0x7));
			}
		}
	}
	...
	private void recoverSmallLcd() {
		if (GlobalDef.mIs8600) {
			if (mBtPhoneStatus == MyCmd.PhoneStatus.PHONE_ON) {					
				GlobalDef.setSmallLcd("BT");	
				GlobalDef.setSmallLcdIcon("0");
			} else {
				setSmallLcdEx(mPreLcd);
				GlobalDef.setSmallLcdIcon(mPreIcon);				
			}			
...
//					GlobalDef.setSmallLcdIcon("0x00002000");
				...
				else if (mBtPhoneStatus != MyCmd.PhoneStatus.PHONE_ON
						&& SOURCE_VOL_8600 != source) {					
					mPreIcon =  icon;
				}
				if (!"OFF".equals(s)) {
					GlobalDef.setSmallLcdIcon(icon);
				}
			} else {
				if (mBtPhoneStatus != MyCmd.PhoneStatus.PHONE_ON
						&& !mLcdShowVol) {
					if (mAppSource == source) {
						GlobalDef.setSmallLcd(str);
						if (MyCmd.SOURCE_RADIO == source) {
							...
							GlobalDef.setSmallLcdIcon(icon);
						} else if (MyCmd.SOURCE_MUSIC == source
								|| MyCmd.SOURCE_VIDEO == source
								|| MyCmd.SOURCE_DVD == source) {
							String icon;
							if (MyCmd.SOURCE_MUSIC == source) {
								icon = "0x00080800";
							} else if (MyCmd.SOURCE_VIDEO == source) {
	...
	private void updateSmallLcdTime() {
		...
		if ("OFF".equals(mPreLcd)&& mBtPhoneStatus != MyCmd.PhoneStatus.PHONE_ON
				&& !mLcdShowVol) { // off set time
...
//			byte s = (byte) curDate.getSeconds();
			...
			mHandleModeKey.sendEmptyMessageDelayed(MSG_UPDATE_8600_LCD_TIME,
					1000);
		}
	}
...
//				mHandleModeKey.removeMessages(MSG_UPDATE_8600_LCD_TIME);
	...
	public void setKeepAcc(int i) {
		mMcu.sendCmd(ProtocolAk47.generateProtocol1((byte) 0xc, (byte) 0x9,
				(byte) i));
	}
	...
	private void updateSaveTimeEx(byte[] mcuTime) {
		mNewSaveTime = true;
		Log.d(TAG, "dupdateSaveTimeEx::" + (2000 + mcuTime[2]) + ":"
				+ mcuTime[3] + ":" + mcuTime[4] + ":" + mcuTime[6] + ":"
				+ mcuTime[7] + ":" + mcuTime[8]);
		...
		c.set(2000 + mcuTime[2], mcuTime[3], mcuTime[4], mcuTime[6],
				mcuTime[7], mcuTime[8]);		
	...
	private void sendKeyToLaite(int keycode) {
		...
		Settings.System.putInt(mContext.getContentResolver(),
				CL_CARPLAY_CONTROL, keycode);
		mContext.getContentResolver().notifyChange(
				Settings.System.getUriFor(CL_CARPLAY_CONTROL), null);
	}
...
//			}
			...
			UtilSystem.doRunActivity(mContext, "com.suding.speedplay",
					"com.suding.speedplay.ui.MainActivity");
		}
	}
	...
	private void doSpeechCarPlayLaite() { // laite
		...
		if (top != null && top.contains("com.carletter.car")) {
			...
			UtilSystem.doRunActivity(mContext, "com.carletter.car",
					"com.carletter.car.ui.CarletterActivity");
		}
	}
}
