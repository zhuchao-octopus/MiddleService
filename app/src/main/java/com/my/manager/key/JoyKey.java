...
import com.my.manager.key.TouchKeyProcessor.Keys;
...
public class JoyKey {
	...
	private boolean isContinueNoLongKey(int key) { // only volume now
		if (key == MyCmd.Keycode.VOLUME_DOWN || key == MyCmd.Keycode.VOLUME_UP
				|| key == MyCmd.Keycode.KEY_JOY_LEFT
				|| key == MyCmd.Keycode.KEY_JOY_RIGHT) {
			return true;
		}
		return false;
	}
		...
		if (mJoyKeyType == 0) {
			...
			if (down) {
				...
				if (mDownKey > 0) {
					...
					if ((System.currentTimeMillis() - mDownTime) > LONG_CLICK_TIME) {
						longClick = true;
					}
