...
public class TouchKeyEvent {
	...
	private int x, y, startX, startY, endX, endY, minVolidX, minVolidY,
			maxVolidX, maxVolidY, slideX, slideY;
...
//		Log.d("dd", "startSendLongPress"+mRollMaxTime);
	...
	private void startSendLongPressFixKey(int key) {
		...
		long l = SystemClock.uptimeMillis();
		if (mRollMaxTime < ROLL_MAX_TIME) {
			mHandler.sendMessageAtTime(mHandler.obtainMessage(1, key, 0),
					LONG_PRESS_INT_TIME + l);
		}
...
//				showBeep();
			...
			case SLIDE:
				...
				break;
			case QUICK_SLIDE:
				mTouchKeyProcessor.onQueckSlide(startX, startY, endX, endY,
						tmpTime);
				break;
			default:
				break;
	...
	private boolean isVolidPoint() {
		return endX > minVolidX && endY > minVolidY && endX < maxVolidX
				&& endY < maxVolidY && !triggerSlide;
	}
				...
				if (mStartTime == 0) {
					...
					if (pressTime > BEGIN_LONG_PRESS_TIME && !mLongPress) {
						if (mKey == protocol[2]) {
							...
							mLongPress = true;
							mRollMaxTime = 0;
							if (key == MyCmd.Keycode.VOLUME_DOWN
									|| key == MyCmd.Keycode.VOLUME_UP) {
								Util.setBeepToMcu();
					...
					if (pressTime > LONG_PRESS_FOR_POWER){
						if (isSupportPowerKey(protocol[2])){
							key = protocol[2];
							Util.setBeepToMcu();
							mTouchKeyProcessor.onSendFixKey((byte)key, mLongPress);	
							clear();
