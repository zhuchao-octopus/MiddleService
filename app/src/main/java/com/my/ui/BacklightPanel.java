...
public class BacklightPanel extends Handler {
	...
	private static String TAG = "BacklightPanel";
	...
	private BacklightUI[] mBacklightUI = new BacklightUI[BacklightUI.MAX_DISPLAY];
	...
	private void initDefalutScreen1Backlight(){
		...
		if (brightness != 0) {
			Util.setFileValue(GlobalDef.BRIGHTNESS_SCREEN1,
					brightness);
		}
...
//						cmd = 1;
	...
	private void init(){ 
		...
		GlobalDef.mReverseBrightness = SystemConfig.getIntProperty(mContext,
				SystemConfig.KEY_REVERSE_BACKLIGHT);
		GlobalDef.mReverseContrast = SystemConfig.getIntProperty(mContext,
...
//				SystemConfig.setIntProperty(mContext, AutoIlluminManager.DAY_BRIGHTNESS, -1);
		...
		Util.setProperty(AKProperty.PROPERTY_ILL_STATE, "" + mSetIll);
...
//			brightness = (((brightness * 20 * 100) / 255) / 100);
			...
			GlobalDef.mReverseBrightness = (brightness);
	...
	private void setContrastInner(int contrast, int type) {
		if (type == -4) {
			...
			GlobalDef.mReverseContrast = (contrast);
			Util.setFileValue(GlobalDef.BRIGHTNESS_CONTRAST, contrast);
