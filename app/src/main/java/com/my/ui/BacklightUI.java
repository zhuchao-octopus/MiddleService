...
public class BacklightUI extends UIBase {
	...
	public BacklightUI(Context context, View view, int index) {
		...
		mContext = context;
		init();
		mLevel = (SeekBar) mMainView.findViewById(R.id.level);
		mLevelBacklight = (TextView) mMainView.findViewById(R.id.backlight_text);
		ImageView mLargeStreamIcon = (ImageView) mMainView
	...
	public void setType(int t) {
		...
		if (t == -4) {
			...
			mLevelConrast = ((TextView)mMainView.findViewById(R.id.contrast_text));
			...
			ImageView contrastIcon = (ImageView) mMainView
					.findViewById(R.id.contrast_icon);
	...
	private void prepareHide() {
		...
		mHandler.sendMessageDelayed(
				mHandler.obtainMessage(MSG_HIDE, mDisplayIndex, 0),
				DELAY_HIDE_TIME);
	}
	...
	private void doShow() {
		...
		if (mType == 0) {
			...
			int s = SystemConfig.getIntProperty(mContext,
			...
			if (s == 0) {
				s = GlobalDef.CVBS_DEFALUT_BRIGHTNESS;
			}
			mLevel.setProgress(s);
			mLevel.setMax(255);
			mLevelBacklight.setText(s+"");
			mLevelBacklight.setVisibility(View.VISIBLE);
			if (mType == -4) {
				s = SystemConfig.getIntProperty(mContext,
						SystemConfig.KEY_REVERSE_CONTRAST);
				if (s == 0) {
					s = GlobalDef.CVBS_DEFALUT_BRIGHTNESS;
				}
				mLevelConrast.setText(s+"");
