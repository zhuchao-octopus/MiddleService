...
//		}
	...
	public void init(int width, int height) {
		...
		double h = ((show_w / 2) / Math.tan(radian_a));		
		car_d -= h;
	...
	private void init_deult_x(double angle) {
		...
		{
			step_small = 0.04;
			step_small_num = show_l_num / 4;
		} 
		else
		{
			step_small = 0.1;
			step_small_num = show_l_num / 40;
		}
		...
		if (angle > 0) // 右转
		{
			...
			for (i = 0; i < show_l_num; i++) {
				if (i < step_small_num) {
					xy_l_real_x[i] = (car_w / 2) - (i * step_small);
				} else {
					xy_l_real_x[i] = (car_w / 2)
							- (step_small_num * step_small)
							- ((i - step_small_num) * step);
			...
			for (i = 0; i < show_l_num; i++) {
				if (i < step_small_num) {
					xy_r_real_x[i] = 0 - i * step_small - (car_w / 2);
				} else {
					xy_r_real_x[i] = 0
							- ((step_small_num * step_small) + ((i - step_small_num) * step))
							- (car_w / 2);
				}

			}
		} else if (angle == 0){
			step_small = SAWTOOTH_STEP3/(show_l_num-1);
			for (i = 0; i < show_l_num; i++) {
				xy_l_real_x[i] = (car_w / 2);
				xy_r_real_x[i] = -(car_w / 2);
...
//								+ " xy_l_real_x[" + i + "]:" + xy_l_real_x[i]);
	...
	private void caculate_virtual_point(double angle) {
		...
		for (i = 0; i < show_l_num; i++) {
			...
			Yr = line_side
					* (Math.sin(radian_c1) / Math.sin(radian_c1 + radian_c2));
...
//			}
	...
	public void set_track_param(double a, double h, double b, double l,
			double w, double d) {
		/*
		 * static double angle_2a = 120 / 2; //摄像头可视角度2a, 
		 * static int camera_h = * 800; //摄像头距离地面距离h 
...
//	}
	...
	private int[] sawtooth_steps = new int[] { SAWTOOTH_STEP1-SAWTOOTH_INTERVERL, SAWTOOTH_STEP1, SAWTOOTH_STEP1 + SAWTOOTH_INTERVERL,
			SAWTOOTH_STEP2-SAWTOOTH_INTERVERL2, SAWTOOTH_STEP2, SAWTOOTH_STEP2 + SAWTOOTH_INTERVERL2, 
			SAWTOOTH_STEP3 };
	public int[] sawtooth_l_point = new int[SAWTOOTH_NUM];
	public int[] sawtooth_r_point = new int[SAWTOOTH_NUM];
	private double[] sawtooth_tan = new double[SAWTOOTH_NUM];
	...
	private void caculte_sawtooth_point(double angle) {
		...
		for (j = 0; j < SAWTOOTH_NUM; ++j) {
			...
			if (i < show_l_num) {
				if (i > 0) {
					if (double_abs(tan_d2 - tan_d) > double_abs(tan_d2_prev
							- tan_d)) 
					{
...
//			}
	...
	private static String SAVE_DATA = "BackTrack";
	...
	private static void saveData(Context c, String s, long v) {
		SharedPreferences.Editor sharedata = c.getSharedPreferences(SAVE_DATA,
				0).edit();
