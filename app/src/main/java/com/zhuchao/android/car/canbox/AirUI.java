...
public class AirUI extends UIBase implements View.OnClickListener {
            ...
            if (mCanbox.mWindMaxStep != 0 && mCanbox.mWindMaxStep <= 0xc) {
                for (int i = 0; i < 0xc; i++) {
                    if (i < mCanbox.mWindMaxStep) {
                        mMainView.findViewById(R.id.wind_rate_1 + i).setVisibility(View.VISIBLE);
                    } else {
                        mMainView.findViewById(R.id.wind_rate_1 + i).setVisibility(View.GONE);
                    }
                }
            }
        }
    ...
    private void setStyle(int style) {
        mStyle = style;
        if (style == STYLE_ALL) {
            mMainView.findViewById(R.id.control_pannel).setVisibility(View.VISIBLE);
            // setViewVisible(mMainView, R.id.control_pannel, 1);
        } else {
            mMainView.findViewById(R.id.control_pannel).setVisibility(View.INVISIBLE);
    ...
    void setAirCondtionTitle(View view) {
        ...
        if ((mAirData[4] & 0x80) != 0) {
            ...
            ((ImageView) view.findViewById(R.id.air_title_ce_inner_loop)).getDrawable().setLevel(2);
        } else {
            if ((mAirData[0] & 0x20) != 0) {// inner loop,loop a
                ((ImageView) view.findViewById(R.id.inner_loop)).getDrawable().setLevel(1);
                ((ImageView) view.findViewById(R.id.air_title_ce_inner_loop)).getDrawable().setLevel(1);

            } else {
                ((ImageView) view.findViewById(R.id.inner_loop)).getDrawable().setLevel(0);
                ((ImageView) view.findViewById(R.id.air_title_ce_inner_loop)).getDrawable().setLevel(0);
    ...
    void setAirCondtionWind(View view) {
        ...
        if ((mAirData[1] & 0x40) != 0) {
            ...
            view.findViewById(R.id.wind_horizontal12).setVisibility(View.VISIBLE);
            ...
            view.findViewById(R.id.wind_horizontal12).setVisibility(View.INVISIBLE);
        ...
        if ((mAirData[1] & 0x0F) >= 7 && (mAirData[1] & 0x0F) <= 0xc) {
            view.findViewById(R.id.wind_rate_1 + (mAirData[1] & 0x0F) - 1).setVisibility(View.VISIBLE);
        }

        setTextViewText(view, R.id.wind_status, String.valueOf(mAirData[1] & 0x0F));
    }
    ...
    String getAirTemperature(View view, float temperature) {
        ...
        boolean isCentigradeUnit = (mAirData[5] & 0x1) == 0;
        if (isCentigradeUnit) {
            return String.format("%.1f%s", temperature / 2, view.getResources().getString(R.string.temp_unic_centigrade));
        } else {
            return String.format("%d%s", (int) temperature, view.getResources().getString(R.string.temp_unic_fahrenheit));
        }
