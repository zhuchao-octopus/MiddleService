            ...
            switch (i) {
                case 0:
                    for (int j = 0; j < mRadarDataFront[r]; ++j) {
                        canvas.drawLine(mRadarFrontStartX[i] + RADAR_BAR_INTERVAL * (RADAR_BAR_NUM - j) + 3, mRadarBackStartY[i] - j, mRadarFrontStartX[i] + RADAR_BAR_INTERVAL * (RADAR_BAR_NUM - j), mRadarBackStartY[i] + RADAR_BAR_HEIGHT - j, mPaint);
                    }
                    break;
                case 1:
                case 2:
                    for (int j = 0; j < mRadarDataFront[r]; ++j) {
                        canvas.drawLine(mRadarFrontStartX[i] + RADAR_BAR_INTERVAL * (RADAR_BAR_NUM - j), mRadarBackStartY[i], mRadarFrontStartX[i] + RADAR_BAR_INTERVAL * (RADAR_BAR_NUM - j), mRadarBackStartY[i] + RADAR_BAR_HEIGHT, mPaint);
                    }
                    break;
                case 3:
                    for (int j = 0; j < mRadarDataFront[r]; ++j) {
                        canvas.drawLine(mRadarFrontStartX[i] + RADAR_BAR_INTERVAL * (RADAR_BAR_NUM - j), mRadarBackStartY[i] + j, mRadarFrontStartX[i] + RADAR_BAR_INTERVAL * (RADAR_BAR_NUM - j) + 3, mRadarBackStartY[i] + RADAR_BAR_HEIGHT + j, mPaint);
                    }
                    break;
            }
        }
            ...
            switch (i) {
                case 0:
                    for (int j = 0; j < mRadarDataBack[r]; ++j) {
                        canvas.drawLine(mRadarBackStartX[i] + RADAR_BAR_INTERVAL * j - 3, mRadarBackStartY[i] - j, mRadarBackStartX[i] + RADAR_BAR_INTERVAL * j, mRadarBackStartY[i] + RADAR_BAR_HEIGHT - j, mPaint);
                    }
                    break;
                case 1:
                case 2:
                    for (int j = 0; j < mRadarDataBack[r]; ++j) {
                        canvas.drawLine(mRadarBackStartX[i] + RADAR_BAR_INTERVAL * j, mRadarBackStartY[i], mRadarBackStartX[i] + RADAR_BAR_INTERVAL * j, mRadarBackStartY[i] + RADAR_BAR_HEIGHT, mPaint);
                    }
                    break;
                case 3:
                    for (int j = 0; j < mRadarDataBack[r]; ++j) {
                        canvas.drawLine(mRadarBackStartX[i] + RADAR_BAR_INTERVAL * j, mRadarBackStartY[i] + j, mRadarBackStartX[i] + RADAR_BAR_INTERVAL * j - 3, mRadarBackStartY[i] + RADAR_BAR_HEIGHT + j, mPaint);
                    }
                    break;
            }
