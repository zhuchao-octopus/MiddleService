...
public class VWMQBRaise extends Canbox {
...
//				}
		...
		case 0x40:			
			if (data[2] == (byte) 0x40) {
				...
				if ((data[3] & 0x5) == 0x5 && ((data[6] & 0x70) != 0x70)) {
				...
				sendToReverseUI(data[2] & 0xff, (data[4] & 0xff)
						| (data[5] & 0xff) << 8 | (data[6] & 0xff) << 16);
			}
			break;
		case 0x68:
			if (((data[2] & 0xf) != 0) || ( data[3] == 2)
					|| ( data[3] == 3)
					|| ( data[3] == 4)) {
		...
		if (data[0] == 0x40 || data[0] == 0x41 || data[0] == 0x50 || data[0] == 0x63|| data[0] == 0x62|| data[0] == 0x60
				 || data[0] == 0x25 || data[0] == 0x16
				|| data[0] == 0x65|| data[0] == 0x68|| data[0] == 0x66
				|| data[0] == 0x51|| data[0] == 0x52) {
			sendCanboxInfo("com.canboxsetting", data);
		}
	}
	private byte []mBuf;
	...
	public void setMediaMoreInfo(int source, int play, int total, int time,
		...
		if (MyCmd.SOURCE_DVD == source) {
			...
			mData = new byte[] { (byte) 0xc0, 0x8, s, (byte)0xff, 
		...
		Util.doSleep(5);
		String str = h + ":" + min + ":" + sec;
		sendId3((byte) 0x73, str);
		// }
	}
...
//					data[3 + i] = n[i];
	...
	private void sendToReverseUI(int data1, int data2) {
		...
		Handler handler = getHandler("Reverse");
		if (null != handler) {
			handler.sendMessage(handler.obtainMessage(CANBOX_VW_RAISE_UI_DATA,
					data1, data2 ));
		}
	}
	...
	public void sendReverseCmd(int cmd) {
		byte[] buf = new byte[] { (byte) ((cmd & 0xff0000) >> 16), 0x2,
				(byte) ((cmd & 0xff00) >> 8), (byte) ((cmd & 0xff) >> 0) };
	...
	public void updateCompass(int compass) {		
		...
		byte[] buf = new byte[] { (byte) (0xa7), 0x3,(byte) (direction&0xff),
				(byte) ((compass & 0xff) >> 0), (byte) ((compass & 0xff00) >> 8) };
