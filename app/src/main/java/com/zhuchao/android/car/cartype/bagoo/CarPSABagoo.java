...
public class CarPSABagoo extends Canbox {
    ...
    private void parseACInfo(byte[] data, int len) {
        ...
        airData[0] = (byte) ((data[2] & 0x80) | (data[2] & 0x40) | ((data[2] & 0x1) << 5) | ((data[2] & 0x20) >> 1) | ((data[2] & 0x10) >> 1) | ((data[2] & 0x8) >> 2) | ((data[2] & 0x4) >> 2));

        airData[1] = 0;
        if (((data[3] & 0xf0) == 0x40) || ((data[3] & 0xf0) == 0x60) || ((data[3] & 0xf0) == 0x70) || ((data[3] & 0xf0) == 0x80)) {
            airData[1] |= 0x80;
        }

        if (((data[3] & 0xf0) == 0x30) || ((data[3] & 0xf0) == 0x50) || ((data[3] & 0xf0) == 0x70) || ((data[3] & 0xf0) == 0x80)) {
            airData[1] |= 0x40;
        }

        if (((data[3] & 0xf0) == 0x20) || ((data[3] & 0xf0) == 0x50) || ((data[3] & 0xf0) == 0x60) || ((data[3] & 0xf0) == 0x80)) {
            airData[1] |= 0x20;
        }
        ...
        airData[6] = 0;
        if (((data[3] & 0xf) == 0x4) || ((data[3] & 0xf) == 0x6) || ((data[3] & 0xf) == 0x7) || ((data[3] & 0xf) == 0x8)) {
            airData[6] |= 0x80;
        }

        if (((data[3] & 0xf) == 0x3) || ((data[3] & 0xf) == 0x5) || ((data[3] & 0xf) == 0x7) || ((data[3] & 0xf) == 0x8)) {
            airData[6] |= 0x40;
        }

        if (((data[3] & 0xf) == 0x2) || ((data[3] & 0xf) == 0x5) || ((data[3] & 0xf) == 0x6) || ((data[3] & 0xf) == 0x8)) {
            airData[6] |= 0x20;
        }
        ...
        if (null != handler) {
            handler.sendMessage(handler.obtainMessage(CANBOX_RETURN_AIR, 1, 0, airData));
        }
    }
