...
public class ChuanQiRaise extends Canbox {
    ...
    private byte[] getCarTypeCmd() {
        byte[] cmd = new byte[]{(byte) 0xee, 0x02, 2, 0};
    ...
    public void parseACInfo(byte[] data) {
        ...
        airData[0] = (byte) (((data[3] & 0x40) >> 5) | ((data[3] & 0x20) >> 5) | ((data[4] & 0x40) >> 4) | ((data[3] & 0x04) << 1) | ((data[3] & 0x01) << 6));

        if (((data[3] & 0x80) == 0)) {
            airData[0] |= 0x20;
        }

        airData[1] = (byte) (((data[3] & 0x08) << 3) | ((data[3] & 0x10) << 1) | ((data[4] & 0xf) << 0));
    ...
    public void setPhone(int status, String num) {// default is simple box
        ...
        if (mPhoneStatus != HFP_INFO_CALLING && status == HFP_INFO_CALLING) {
            mCallingTime = 0;
