...
public class AutoIlluminManager {
    ...
    public void init(Context c) {
        ...
        if (value != 0) {
            ...
            try {
                ...
                com.zhuchao.android.car.ui.BacklightPanel.setBacklightBrightness(c, value);
    ...
    private final Handler mAutoIlluminHandler = new Handler(Objects.requireNonNull(Looper.myLooper())) {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == MSG_UPDATE_AUTO_ILLUMIN) {
                updateIlluminMode();
                mAutoIlluminHandler.sendEmptyMessageDelayed(MSG_UPDATE_AUTO_ILLUMIN, UPDATE_AUTO_ILLUMIN_TIME);
            }
        }
    };

    public void updateIlluminMode() {
        boolean autoIlluminEnable = (0 != Settings.Global.getInt(mContext.getContentResolver(), AUTO_ILL_ENABLE, 0));
    ...
    private int getIlluminMode() {
        ...
        if ((mode > ILLUMIN_MODE_DAY) || (mode < ILLUMIN_MODE_BY_CAR)) {
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        void startObserving() {
            ...
            resolver.registerContentObserver(Settings.Global.getUriFor(AUTO_ILL_ENABLE), false, this);
            resolver.registerContentObserver(Settings.Global.getUriFor(AUTO_ILL_START_HOUR), false, this);
            resolver.registerContentObserver(Settings.Global.getUriFor(AUTO_ILL_START_MINUTE), false, this);
            resolver.registerContentObserver(Settings.Global.getUriFor(AUTO_ILL_STOP_HOUR), false, this);
            resolver.registerContentObserver(Settings.Global.getUriFor(AUTO_ILL_STOP_MINUTE), false, this);
        }
