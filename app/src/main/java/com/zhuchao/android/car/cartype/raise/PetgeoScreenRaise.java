...
public class PetgeoScreenRaise extends Canbox {
    ...
    private void parseACInfo(byte[] data, int len) {
        ...
        Handler handler = getHandler("CanService");
        if (null != handler) {
            handler.sendMessage(handler.obtainMessage(CANBOX_RETURN_AIR, windMode, 0, airData));
        }
        //		}
    }
    ...
    public void parseCanboxData(byte[] data, int len) {
        ...
        switch (data[1]) {
            ...
            case 0x36: {
                ...
                @SuppressLint("DefaultLocale") String s = String.format("%d%s", temp, mContext.getResources().getString(R.string.temp_unic_centigrade));
