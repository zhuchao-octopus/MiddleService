...
public class MinJueRongWeiRaise extends Canbox{
	...
	private final static byte[][] KEYS_WHEEL = {
		...
		{ 0x6, MyCmd.Keycode.MULT_MUTE_AND_BT },
		{ 0x7, MyCmd.Keycode.MODLE },
//		{ 0x8, MyCmd.Keycode },
		{ 0x9, MyCmd.Keycode.BT },
		{ 0x10, MyCmd.Keycode.SPEECH },
		{ 0x11, MyCmd.Keycode.MENU },
		{ 0x12, MyCmd.Keycode.BACK },
		{ 0x13, MyCmd.Keycode.HOME },
		{ 0x14, MyCmd.Keycode.CANBOX_OPEN_AC_VIEW },
		{ 0x15, MyCmd.Keycode.KEY_CAR_SETTING },
		{ 0x16, MyCmd.Keycode.MUTE },
		{ 0x17, MyCmd.Keycode.KEY_360 },
		{ 0x1f, MyCmd.Keycode.NUMBER_STAR },
		{ 0x20, MyCmd.Keycode.ROLL_PREV },
		{ 0x21, MyCmd.Keycode.ROLL_NEXT },
	...
	public void parseACInfo(byte[] data)
	{
		...
		airData[4] = (byte) (((data[7] & 0x80) >>4 )
				| ((data[2] & 0x04) <<5 )
				| ((data[5] & 0x06) >>1 )
				| ((data[5] & 0x18) <<1 ));
		
		airData[7] = (byte) (((data[7] & 0x40) <<1 )
				|  ((data[5] & 0x20) >>5 ));
...
//			Util.zeroBuf(airData);
	...
	public void parseCanboxData(byte[] data, int len) {
		...
		case 0x24:
			...
			break;
		case 0x53:			
			if ("com.canboxsetting/com.canboxsetting.MainActivity"
				...
				if (data[2] < 4 && mDriverMode != data[2]) {// no super sport
					...
					if (!"com.canboxsetting/com.canboxsetting.MainActivity"
						...
						mHandler.sendMessageDelayed(
								mHandler.obtainMessage(0, data), 800);
					} else {
						super.parseCanboxData(data, len);
					}
					mDriverMode = data[2];
