...
public class JeepXinbas extends Canbox {
	...
	{ 0x1, KEY_SOURCE }, { 0x3, AK_KEYPAD_VOLUME_D },
			{ 0x2, AK_KEYPAD_VOLUME_A },
	...
	public void setMediaMoreInfo(int source, int play, int total, int time,
		...
		mData = new byte[] { (byte) 0x84, 0x5, s, 0, (byte) ((play) & 0xFF),
				sec, min };
		...
		if (status == 0x43 || status == 0) {
			...
			if (num != null) {
				try {
					...
					mHandler.sendMessageDelayed(
							mHandler.obtainMessage(SEND_MEDIA_TEXT, data), 30);
				} catch (Exception e) {
