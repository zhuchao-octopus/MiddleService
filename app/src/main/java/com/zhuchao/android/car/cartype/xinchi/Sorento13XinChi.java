...
//		buildCmdOutTemp(5);
	...
	private final static byte[][] KEYS_WHEEL = {
		...
		{ 0xa, MyCmd.Keycode.KEY_DISPLAY },
		{ 0xb, MyCmd.Keycode.BACK },
		{ 0xc, MyCmd.Keycode.SPEECH },
	};
...
//		airData[4] = (byte) (((data[6] & 0x40) >> 3));	
	...
	public int doEQCmd(int cmd, int data) {
		...
		if (cmd == EQ_REQUEST_ALL_MAX) {
			...
			byte[] buf = new byte[] { (byte) 0x93, 0x7, mEQData[5], (byte) (mEQData[4] + 1),
					(byte) (mEQData[3] + 1), (byte) (mEQData[2] + 1),
					(byte) (mEQData[1] + 1), (byte) (mEQData[0] + 1), 0 };
			sendDataToCanbox(buf, buf.length);
...
//			super.returnEQData(EQ_CMD_SET_ALL_DATA, mEQData);
	...
	private void stopEQ() {
		byte[] buf = new byte[] { (byte) 0x93, 0x7, (byte)0x80, (byte) (mEQData[4] + 1),
				(byte) (mEQData[3] + 1), (byte) (mEQData[2] + 1),
				(byte) (mEQData[1] + 1), (byte) (mEQData[0] + 1), 0 };
		sendDataToCanbox(buf, buf.length);
	}
