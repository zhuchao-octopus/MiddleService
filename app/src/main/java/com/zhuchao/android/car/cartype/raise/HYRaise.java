...
public class HYRaise extends Canbox {
    ...
    private void parseACInfo(byte[] data, int len) {
        ...
        airData[1] = (byte) ((data[4] & 0x0f) | ((data[5] & 0x40) << 1) | ((data[5] & 0x10) << 1) | ((data[5] & 0x08) << 3));

        airData[0] = (byte) (((data[5] & 0x80) >> 2) | ((data[5] & 0x20) >> 5) | ((data[5] & 0x04) << 1) | ((data[5] & 0x02) << 1) | ((data[5] & 0x01) << 6));
    ...
    public void parseCanboxData(byte[] data, int len) {
        ...
        switch (data[1]) {
            ...
            case 0x40: {
                ...
                Handler handler = getHandler("Reverse");
                if (null != handler) {
                    handler.sendMessage(handler.obtainMessage(CANBOX_HY_UI_DATA, d360));
                }
                byte reverse = 0;
                if ((d360[0] & 0xC0) != 0x0) {
                    reverse = 1;
    ...
    private void startEQ() {
        ...
        if (CarUtil.getProIndex() <= 0) {
            mEQData[5] = (byte) MachineConfig.getIntProperty2(SettingProperties.CANBOX_EQ_VOLUME);
            if (mEQData[5] == -1) {
                mEQData[5] = 32;
            }
