...
public class FremontXinChi extends Canbox {
    ...
    public void parseACInfo(byte[] data) {
        ...
        mAirDataEx[0] = (byte) ((data[2] & 0xd0) | ((data[2] & 0x01) << 1) | ((data[2] & 0x02) >> 1) | ((data[2] & 0x04) << 3));

        mAirDataEx[1] = (byte) ((data[3] & 0x07) | ((data[4] & 0x40) << 1) | ((data[4] & 0x20) >> 0) | ((data[4] & 0x10) << 1));

        mAirDataEx[4] &= (~0x33);
        mAirDataEx[4] |= (byte) (((data[7] & 0xcc) >> 2));

        mAirDataEx[7] = (byte) (((data[3] & 0x40) << 1));

        mAirDataEx[8] = (byte) (((data[7] & 0x03) << 2) | ((data[7] & 0x30) >> 0));

        mAirDataEx[12] = (byte) (((data[8] & 0x80) >> 7));
    ...
    public void parseACInfoRear(byte[] data) {
        ...
        mAirDataEx[4] &= (~0x08);
        mAirDataEx[4] |= (byte) (((data[2] & 0x20) >> 2));

        mAirDataEx[9] = (byte) ((data[2] & 0x80) | ((data[2] & 0x40) >> 5));
        mAirDataEx[10] = (byte) getACTemp(data[4]);
        mAirDataEx[11] = (byte) ((data[3] & 0x40) | ((data[2] & 0x07) << 0) | ((data[3] & 0x80) >> 2));
        super.parseACInfo(mAirDataEx);
    }
    ...
    public void setMediaMoreInfo(int source, int play, int total, int time, int total_time) {
        ...
        String s = String.format("%03d/%03d %02d:%02d", play, total, time / 60, time % 60, Locale.ENGLISH);
