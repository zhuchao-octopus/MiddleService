...
public class Canbox {
    ...
    */
    ...
    public void byteArrayCopy(byte[] dest, byte[] src, int dStart, int sStart, int len) {
        if (len >= 0) System.arraycopy(src, sStart + 0, dest, 0 + dStart, len);
    }
    ...
    public void autoUpdateCarType() {
        if (mRepeatSendCarType != null) {
            ...
            if (mRepeatCmdTime == -1 || mRepeatCmdTime > 0) {
                ...
                mHandlerRadar.sendEmptyMessageDelayed(REPEAT_SEND_CAR_TYPE, 1000);
            }
        }
    }
    ...
    protected boolean doKeyStudy(int groundId, int value, int status) {
        ...
        if (mMapKeyStudy != null) {
            ...
            switch (status) {
                ...
                case 2:
                    if (mKeyDownMap != 0) {
                        if ((mKeyDownMap & 0xff) == AK_KEYPAD_VOLUME_A || (mKeyDownMap & 0xff) == AK_KEYPAD_VOLUME_D) {
                            doKey((mKeyDownMap & 0xff));
                        } else {
                            if ((System.currentTimeMillis() - mClickTime) > LONG_CLICK_TIME) {
                                if (((mKeyDownMap & 0xff00) >> 8) != 0) {
    ...
    public boolean isShowAir() {
        // if (isAirtContolCar()) {
        return !"com.canboxsetting/com.canboxsetting.CanAirControlActivity".equals(AppConfig.getTopActivity());
        // }
    }
    ...
    public void returnDriveData() {
        if (mRequestDriveData > 0) {
            if (mContext != null) {
                Intent i = new Intent(MyCmd.BROADCAST_SEND_FROM_CAN);
                i.putExtra(MyCmd.EXTRA_COMMON_CMD, MyCmd.Cmd.CANBOX_RETURN_DRIVE_DATA);
    ...
    public void buildCmdAngle(byte cmd, byte style, int max) {
        mIdAngle = (cmd & 0xff) | ((style & 0xff) << 8) | ((max & 0xffff) << 16);
    }

    public void buildCmdDoor(byte cmd, byte style, byte mask, byte dataIndex) {
        mIdDoor = (cmd & 0xff) | ((style & 0xff) << 8) | ((mask & 0xff) << 16) | ((dataIndex & 0xff) << 24);
    }

    public void buildCmdDoor(byte cmd, byte style, byte mask, byte dataIndex, byte subCmd) {
        mIdDoor = (cmd & 0xff) | ((style & 0xff) << 8) | ((mask & 0xff) << 16) | ((dataIndex & 0xf) << 24) | ((dataIndex & 0xf) << 28);
    }
    ...
    public void buildCmdRadarBack(byte cmd, byte style, byte max, byte midMax, byte num) {
        buildCmdRadarBack(cmd, (byte) (style & 0x7 | (midMax << 3)), max, num);
    }

    public void buildCmdRadarFront(byte cmd, byte style, byte max, byte midMax, byte num) {
        buildCmdRadarFront(cmd, (byte) (style & 0x7 | (midMax << 3)), max, num);
    }
    ...
    private void parseRadar(int id, byte[] data, int startIndex, int arrayIndex, int style2) {
        ...
        mRadar[startIndex] = radarChangeStyle(data[arrayIndex + 2], max, style);
        if (num == 4) {
            mRadar[startIndex + 1] = radarChangeStyle(data[arrayIndex + 3], midMax, style);
            mRadar[startIndex + 2] = radarChangeStyle(data[arrayIndex + 4], midMax, style);
            mRadar[startIndex + 3] = radarChangeStyle(data[arrayIndex + 5], max, style);
        } else if (num == 3) {
            if (style2 == 0) {
                mRadar[startIndex + 1] = radarChangeStyle(data[arrayIndex + 3], midMax, style);
                mRadar[startIndex + 2] = radarChangeStyle(data[arrayIndex + 3], midMax, style);
                mRadar[startIndex + 3] = radarChangeStyle(data[arrayIndex + 4], max, style);
            } else if (style2 == 1) {
                mRadar[startIndex + 1] = radarChangeStyle(data[arrayIndex + 3], midMax, style);
                mRadar[startIndex + 2] = radarChangeStyle(data[arrayIndex + 3], midMax, style);
                mRadar[startIndex + 3] = radarChangeStyle(data[arrayIndex + 5], max, style);
            ...
            if (style2 == 0) {
                ...
                mRadar[startIndex + 3] = radarChangeStyle(data[arrayIndex + 3], max, style);
            } else if (style2 == 1) {
                mRadar[startIndex + 1] = radarChangeStyle(data[arrayIndex + 3], max, style);
            }
        } else if (num == 1) {
            mRadar[startIndex + 1] = mRadar[startIndex];
            mRadar[startIndex + 2] = mRadar[startIndex];
            mRadar[startIndex + 3] = mRadar[startIndex];
        }
...
：引擎盖*/
    ...
    private void doKeyRoll(int key, int step) {
        ...
        --step;
        if (step > 0) {
            mHandlerRadar.sendMessageDelayed(mHandlerRadar.obtainMessage(REPEAT_SEND_ROLL_KEY, key, step), 30);
        }
    }
    ...
    private boolean isVolumeKey(int key) {
        return key == MyCmd.Keycode.VOLUME_UP || MyCmd.Keycode.VOLUME_DOWN == key;
    }
    ...
    public void updateOutDoorTemp(int temp) {
        ...
        if (temp == CarUtil.CLEAR_OUT_DOOR_TEMP) {
            ...
            if (mUnit != 1) {
                ...
                s = s + String.format("%d.%d%s", temp / 10, (temp % 10) >= 0 ? (temp % 10) : -(temp % 10), mContext.getResources().getString(R.string.temp_unic_centigrade));
    ...
    public void doCanboxFunctionKey(byte key) {
        if (key == MyCmd.Keycode.CANBOX_OPEN_AC_VIEW) {
        }
    }
