    ...
    private void initUI(int index) {
        ...
        if (mBacklightUI[index] == null) {
            ...
            if (index == 0) {
                ...
                mBacklightUI[index] = BacklightUI.getInstanse(mContext, mView, index);
            } else {
                ...
                if (display.length > 1) {
                    mPresentation = new Presentation(mContext, display[1], R.style.TranslucentTheme);
                    ...
                    mBacklightUI[index] = BacklightUI.getInstanse(mContext, view, index);
                }
        ...
        if (mReceiver == null) {
            mReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    Log.d(TAG, action);
                    if (action.equals(MyCmd.BROADCAST_START_BACKLIGHTSETTINGS_SYSTEM_UI) || action.equals(MyCmd.BROADCAST_START_BACKLIGHTSETTINGS_SETTINGS) || action.equals(MyCmd.BROADCAST_START_BACKLIGHTSETTINGS) || action.equals(MyCmd.BROADCAST_START_BACKLIGHTSETTINGS_COMMON)) {
