...
public class UpdateSimKey extends Canbox {
        ...
        if (data[0] == (byte) 0xe1) {
            switch (data[2]) {
                ...
                case 0x9:// stop
                    ...
                    break;
                case 0xa:
                    mUpdateDialog.setMsg("update success!!!!!!!");
                    break;
            }
        }
    }
    ...
    private void continueUpdate() {
        try {
            if ((buf.length - mSendLen) <= 0) return;
            ...
            data[1] = (byte) (len + 1);
            data[2] = subcmd;
            if (len >= 0) System.arraycopy(buf, mSendLen, data, 3, len);
            mSendLen += len;
            sendDataToCanbox(data, data.length);
            int p = 0;
            p = mSendLen * 100 / buf.length;
            mUpdateDialog.setMsg("update %" + p);
            //MMLog.e("abc", mSendLen + ":" + buf.length + ":" + p);
        } catch (Exception e) {
            MMLog.e("abc", "continuUpdate!!!!!!!!!!err:");
        }
    }
