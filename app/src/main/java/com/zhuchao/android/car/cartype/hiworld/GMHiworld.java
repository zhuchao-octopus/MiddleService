...
public class GMHiworld extends Canbox {
    ...
    public void parseACInfo(byte[] data) {
        ...
        airData[0] = (byte) (((data[2] & 0x10) << 0) | ((data[2] & 0x40) << 1) | ((data[2] & 0x01) << 6) | ((data[3] & 0x10) << 1) | ((data[4] & 0x10) >> 3) | ((data[4] & 0x20) >> 5));
        ...
        airData[5] = (byte) (((data[3] & 0x20) >> 0));

        if (((data[2] & 0x0c) >> 2) == 1) {
            airData[7] |= 0x80;
        }

        airData[4] = (byte) (((data[3] & 0x08) << 4));
        ...
        if ((data[3] & 0x40) == 0) {
            ...
            airData[8] = (byte) (((data[4] & 0x03) << 4));
        }

        if ((data[3] & 0x80) == 0) {
            airData[4] |= (byte) (((data[4] & 0x0c) >> 2));
        } else {
            airData[8] |= (byte) (((data[4] & 0x0c) << 0));
        }
