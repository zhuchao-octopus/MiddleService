...
public class FiatEGEARaise extends Canbox {
    ...
    private final static byte[][] KEYS_WHEEL = {
            {0x1, AK_KEYPAD_VOLUME_A}, {0x2, AK_KEYPAD_VOLUME_D}, {0x3, KEY_NEXTSONG}, {0x4, KEY_PREVIOUSSONG}, {0x5, KEY_BT},
    ...
    public void setMediaMoreInfo(int source, int play, int total, int time, int total_time) {
        ...
        if (mPhoneStatus < HFP_INFO_CALLED) {
            data = new byte[]{
                    (byte) 0xc0, 0x6, s, (byte) ((total & 0xff00) >> 8), (byte) ((total & 0xff) >> 0), (byte) ((play & 0xff00) >> 8), (byte) ((play & 0xff) >> 0), 1
            };
            sendDataToCanbox(data, data.length);
        }
    }
    ...
    public void setMediaSrc(int source, byte type, byte[] b) {
        ...
        data = new byte[]{
                (byte) 0xc0, 0x6, s, b[2], b[1], (byte) (b[3] + 1), b[0], 0
        };
        sendDataToCanbox(data, data.length);
    }
    ...
    public void updateTime() {
        ...
        byte[] buf = new byte[]{
                (byte) 0xa7, 0x07, (byte) DecimaltoBCD(y / 100), (byte) DecimaltoBCD(y % 100), (byte) DecimaltoBCD(mon), (byte) DecimaltoBCD(d), (byte) DecimaltoBCD(h), (byte) DecimaltoBCD(m),
                (byte) DecimaltoBCD(s)
        };
